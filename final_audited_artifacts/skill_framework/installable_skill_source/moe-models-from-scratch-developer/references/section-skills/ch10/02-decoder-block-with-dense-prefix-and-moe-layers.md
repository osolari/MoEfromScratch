---
name: moe-ch10-s02-decoder-block-with-dense-prefix-and-moe-la
description: use when developing section 10.2, decoder block with dense prefix and moe layers, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 10.2 - Decoder block with dense prefix and MoE layers
...
Develop only Section 10.2 of *MoE Models from Scratch*: **Decoder block with dense prefix and MoE layers**.

- Chapter: 10. Assembling MiniDeepSeekMoE end to end
- Target section file: `latex_book_skeleton/chapters/ch10/sections/02-decoder-block-with-dense-prefix-and-moe-layers.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch10/fig-decoder-block-with-dense-prefix-and-moe-layers.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch10/tab-decoder-block-with-dense-prefix-and-moe-layers.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch10/eq-decoder-block-with-dense-prefix-and-moe-layers.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch10/lst-decoder-block-with-dense-prefix-and-moe-layers.tex`
- Required diagnostic: List layer types during model initialization.
...
```text
Develop Section 10.2, "Decoder block with dense prefix and MoE layers", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 10.2 - Decoder block with dense prefix and MoE layers
...
| Chapter | 10. Assembling MiniDeepSeekMoE end to end |
| Section | 10.2. Decoder block with dense prefix and MoE layers |
| Section label slug | `ch10-decoder-block-with-dense-prefix-and-moe-layers` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch10/sections/02-decoder-block-with-dense-prefix-and-moe-layers.tex` |
| Planned section include | `\input{chapters/ch10/sections/02-decoder-block-with-dense-prefix-and-moe-layers}` |
| Previous handoff | Chapter 10.1 - Configuration objects and model variants |
| Next handoff | Chapter 10.3 - Forward pass outputs and loss dictionary |
...
**Mini-example to anchor the section:** Use one dense prefix layer followed by MoE blocks in the default config.
...
**This section's bridge sentence:** The model forward pass must carry routing metrics out of each MoE layer.
...
1. **Opening paragraph.** State the concrete problem: Assemble layers so early dense computation can precede sparse MoE layers if configured. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use one dense prefix layer followed by MoE blocks in the default config.
3. **Visual explanation.** Insert `\input{figures/ch10/fig-decoder-block-with-dense-prefix-and-moe-layers}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch10/eq-decoder-block-with-dense-prefix-and-moe-layers}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch10/lst-decoder-block-with-dense-prefix-and-moe-layers}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch10/tab-decoder-block-with-dense-prefix-and-moe-layers}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch10/fig-decoder-block-with-dense-prefix-and-moe-layers.tex` |
| Include command | `\input{figures/ch10/fig-decoder-block-with-dense-prefix-and-moe-layers}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch10-decoder-block-with-dense-prefix-and-moe-layers` |
| Caption intent | Stack of decoder blocks showing dense prefix and repeated MoE layers. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Stack of decoder blocks showing dense prefix and repeated MoE layers.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch10/tab-decoder-block-with-dense-prefix-and-moe-layers.tex` |
| Include command | `\input{tables/ch10/tab-decoder-block-with-dense-prefix-and-moe-layers}` |
| Label | `tab:ch10-decoder-block-with-dense-prefix-and-moe-layers` |
| Caption intent | Layer type by index for smoke, default, and larger teaching configs. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Decoder block with dense prefix and MoE layers"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch10/eq-decoder-block-with-dense-prefix-and-moe-layers.tex` |
| Include command | `\input{equations/ch10/eq-decoder-block-with-dense-prefix-and-moe-layers}` |
| Label | `eq:ch10-decoder-block-with-dense-prefix-and-moe-layers` |
| Equation intent | block(x) = x + attention(norm(x)); x = x + ffn_or_moe(norm(x)). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch10/lst-decoder-block-with-dense-prefix-and-moe-layers.tex` |
| Include command | `\input{listings/ch10/lst-decoder-block-with-dense-prefix-and-moe-layers}` |
| Label | `lst:ch10-decoder-block-with-dense-prefix-and-moe-layers` |
| Listing intent | DecoderBlock factory that chooses DenseFFN or MiniDeepSeekMoELayer. |

**Code objects to develop or reference:**
- `DecoderBlock`
- `dense prefix/MoE layer schedule`
...
2. Add only the minimal new object or function needed for **Decoder block with dense prefix and MoE layers**.
3. Write a CPU smoke test using the mini-example: Use one dense prefix layer followed by MoE blocks in the default config.
...
**Expected code deliverable:** DecoderBlock factory that chooses DenseFFN or MiniDeepSeekMoELayer.
