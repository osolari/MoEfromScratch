---
name: moe-ch09-s03-biased-selection-and-unbiased-combine
description: use when developing section 9.3, biased selection and unbiased combine, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 9.3 - Biased selection and unbiased combine
...
Develop only Section 9.3 of *MoE Models from Scratch*: **Biased selection and unbiased combine**.

- Chapter: 9. Auxiliary-loss-free load balancing
- Target section file: `latex_book_skeleton/chapters/ch09/sections/03-biased-selection-and-unbiased-combine.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch09/fig-biased-selection-and-unbiased-combine.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch09/tab-biased-selection-and-unbiased-combine.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch09/eq-biased-selection-and-unbiased-combine.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch09/lst-biased-selection-and-unbiased-combine.tex`
- Required diagnostic: Unit test that changing bias can change expert IDs while gate values still come from raw scores.
...
```text
Develop Section 9.3, "Biased selection and unbiased combine", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 9.3 - Biased selection and unbiased combine

Develop only Chapter 9.3, **Biased selection and unbiased combine**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch09/03-biased-selection-and-unbiased-combine.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch09/sections/03-biased-selection-and-unbiased-combine.tex`
...
# Detailed section development brief: Chapter 9.3 - Biased selection and unbiased combine
...
| Chapter | 9. Auxiliary-loss-free load balancing |
| Section | 9.3. Biased selection and unbiased combine |
| Section label slug | `ch09-biased-selection-and-unbiased-combine` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch09/sections/03-biased-selection-and-unbiased-combine.tex` |
| Planned section include | `\input{chapters/ch09/sections/03-biased-selection-and-unbiased-combine}` |
| Previous handoff | Chapter 9.2 - Router bias as non-trainable state |
| Next handoff | Chapter 9.4 - Dynamic bias update from expert load |
...
**Mini-example to anchor the section:** A token selects experts using score+bias, then gates are computed from the original selected scores.
...
**This section's bridge sentence:** The bias vector needs an update rule based on observed expert load.
...
1. **Opening paragraph.** State the concrete problem: Implement the central invariant of the final reference model. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: A token selects experts using score+bias, then gates are computed from the original selected scores.
3. **Visual explanation.** Insert `\input{figures/ch09/fig-biased-selection-and-unbiased-combine}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch09/eq-biased-selection-and-unbiased-combine}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch09/lst-biased-selection-and-unbiased-combine}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch09/tab-biased-selection-and-unbiased-combine}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch09/fig-biased-selection-and-unbiased-combine.tex` |
| Include command | `\input{figures/ch09/fig-biased-selection-and-unbiased-combine}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch09-biased-selection-and-unbiased-combine` |
| Caption intent | Two score streams: biased scores for top-k, unbiased scores for gate normalization. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Two score streams: biased scores for top-k, unbiased scores for gate normalization.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch09/tab-biased-selection-and-unbiased-combine.tex` |
| Include command | `\input{tables/ch09/tab-biased-selection-and-unbiased-combine}` |
| Label | `tab:ch09-biased-selection-and-unbiased-combine` |
| Caption intent | Selection tensor versus combine tensor and where each is used. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Biased selection and unbiased combine"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch09/eq-biased-selection-and-unbiased-combine.tex` |
| Include command | `\input{equations/ch09/eq-biased-selection-and-unbiased-combine}` |
| Label | `eq:ch09-biased-selection-and-unbiased-combine` |
| Equation intent | idx = topk(score + b); gate = normalize(score[idx]). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch09/lst-biased-selection-and-unbiased-combine.tex` |
| Include command | `\input{listings/ch09/lst-biased-selection-and-unbiased-combine}` |
| Label | `lst:ch09-biased-selection-and-unbiased-combine` |
| Listing intent | Router forward pass returning topk_idx from biased scores and gates from unbiased selected scores. |

**Code objects to develop or reference:**
- `select_with_bias`
- `combine_with_unbiased_scores`
...
2. Add only the minimal new object or function needed for **Biased selection and unbiased combine**.
3. Write a CPU smoke test using the mini-example: A token selects experts using score+bias, then gates are computed from the original selected scores.
...
**Expected code deliverable:** Router forward pass returning topk_idx from biased scores and gates from unbiased selected scores.
