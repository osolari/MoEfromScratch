---
name: moe-ch09-s05-comparing-balancing-methods
description: use when developing section 9.5, comparing balancing methods, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 9.5 - Comparing balancing methods
...
Develop only Section 9.5 of *MoE Models from Scratch*: **Comparing balancing methods**.

- Chapter: 9. Auxiliary-loss-free load balancing
- Target section file: `latex_book_skeleton/chapters/ch09/sections/05-comparing-balancing-methods.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch09/fig-comparing-balancing-methods.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch09/tab-comparing-balancing-methods.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch09/eq-comparing-balancing-methods.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch09/lst-comparing-balancing-methods.tex`
- Required diagnostic: Produce a table with LM loss, aux loss if used, load balance score, entropy, and drop rate.
...
```text
Develop Section 9.5, "Comparing balancing methods", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 9.5 - Comparing balancing methods

Develop only Chapter 9.5, **Comparing balancing methods**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch09/05-comparing-balancing-methods.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch09/sections/05-comparing-balancing-methods.tex`
...
# Detailed section development brief: Chapter 9.5 - Comparing balancing methods
...
| Chapter | 9. Auxiliary-loss-free load balancing |
| Section | 9.5. Comparing balancing methods |
| Section label slug | `ch09-comparing-balancing-methods` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch09/sections/05-comparing-balancing-methods.tex` |
| Planned section include | `\input{chapters/ch09/sections/05-comparing-balancing-methods}` |
| Previous handoff | Chapter 9.4 - Dynamic bias update from expert load |
| Next handoff | Chapter 9.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Run three short smoke experiments with identical seeds and report the same diagnostics.
...
**This section's bridge sentence:** The next chapter assembles the full MiniDeepSeekMoE model around this final layer.
...
1. **Opening paragraph.** State the concrete problem: Design a fair chapter experiment comparing no balance, auxiliary loss, and router-bias balancing. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Run three short smoke experiments with identical seeds and report the same diagnostics.
3. **Visual explanation.** Insert `\input{figures/ch09/fig-comparing-balancing-methods}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch09/eq-comparing-balancing-methods}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch09/lst-comparing-balancing-methods}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch09/tab-comparing-balancing-methods}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch09/fig-comparing-balancing-methods.tex` |
| Include command | `\input{figures/ch09/fig-comparing-balancing-methods}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch09-comparing-balancing-methods` |
| Caption intent | Three-panel comparison of expert load histograms and validation loss curves. |
...
**Figure content plan:** Three-panel comparison of expert load histograms and validation loss curves.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch09/tab-comparing-balancing-methods.tex` |
| Include command | `\input{tables/ch09/tab-comparing-balancing-methods}` |
| Label | `tab:ch09-comparing-balancing-methods` |
| Caption intent | Experiment matrix with balancing method, objective terms, update rule, and expected diagnostics. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Comparing balancing methods"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch09/eq-comparing-balancing-methods.tex` |
| Include command | `\input{equations/ch09/eq-comparing-balancing-methods}` |
| Label | `eq:ch09-comparing-balancing-methods` |
| Equation intent | Balance score based on max_load/mean_load or coefficient of variation. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch09/lst-comparing-balancing-methods.tex` |
| Include command | `\input{listings/ch09/lst-comparing-balancing-methods}` |
| Label | `lst:ch09-comparing-balancing-methods` |
| Listing intent | run_balancing_ablation.py command presets. |

**Code objects to develop or reference:**
- `balancing_method_ablation`
...
2. Add only the minimal new object or function needed for **Comparing balancing methods**.
3. Write a CPU smoke test using the mini-example: Run three short smoke experiments with identical seeds and report the same diagnostics.
...
**Expected code deliverable:** run_balancing_ablation.py command presets.
