---
name: moe-ch06-s01-flattened-tokens-as-the-dispatch-unit
description: use when developing section 6.1, flattened tokens as the dispatch unit, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 6.1 - Flattened tokens as the dispatch unit
...
Develop only Section 6.1 of *MoE Models from Scratch*: **Flattened tokens as the dispatch unit**.

- Chapter: 6. Vectorized dispatch, capacity, and routing efficiency
- Target section file: `latex_book_skeleton/chapters/ch06/sections/01-flattened-tokens-as-the-dispatch-unit.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch06/fig-flattened-tokens-as-the-dispatch-unit.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch06/tab-flattened-tokens-as-the-dispatch-unit.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch06/eq-flattened-tokens-as-the-dispatch-unit.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch06/lst-flattened-tokens-as-the-dispatch-unit.tex`
- Required diagnostic: Round-trip flatten/unflatten equality check.
...
```text
Develop Section 6.1, "Flattened tokens as the dispatch unit", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 6.1 - Flattened tokens as the dispatch unit

Develop only Chapter 6.1, **Flattened tokens as the dispatch unit**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch06/01-flattened-tokens-as-the-dispatch-unit.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch06/sections/01-flattened-tokens-as-the-dispatch-unit.tex`
...
# Detailed section development brief: Chapter 6.1 - Flattened tokens as the dispatch unit
...
| Chapter | 6. Vectorized dispatch, capacity, and routing efficiency |
| Section | 6.1. Flattened tokens as the dispatch unit |
| Section label slug | `ch06-flattened-tokens-as-the-dispatch-unit` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch06/sections/01-flattened-tokens-as-the-dispatch-unit.tex` |
| Planned section include | `\input{chapters/ch06/sections/01-flattened-tokens-as-the-dispatch-unit}` |
| Previous handoff | Chapter 5.6 - Chapter summary and handoff |
| Next handoff | Chapter 6.2 - Expert batch construction |
...
**Mini-example to anchor the section:** Flatten B=2, T=3, D=4 into N=6 token rows and recover B,T,D afterward.
...
**This section's bridge sentence:** Flat token rows can be sorted or bucketed by expert assignment.
...
1. **Opening paragraph.** State the concrete problem: Standardize on N=B*T flattened tokens so routing code ignores batch layout until the final reshape. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Flatten B=2, T=3, D=4 into N=6 token rows and recover B,T,D afterward.
3. **Visual explanation.** Insert `\input{figures/ch06/fig-flattened-tokens-as-the-dispatch-unit}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch06/eq-flattened-tokens-as-the-dispatch-unit}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch06/lst-flattened-tokens-as-the-dispatch-unit}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch06/tab-flattened-tokens-as-the-dispatch-unit}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch06/fig-flattened-tokens-as-the-dispatch-unit.tex` |
| Include command | `\input{figures/ch06/fig-flattened-tokens-as-the-dispatch-unit}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch06-flattened-tokens-as-the-dispatch-unit` |
| Caption intent | Batch-sequence grid flattened into a token table for routing. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Batch-sequence grid flattened into a token table for routing.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch06/tab-flattened-tokens-as-the-dispatch-unit.tex` |
| Include command | `\input{tables/ch06/tab-flattened-tokens-as-the-dispatch-unit}` |
| Label | `tab:ch06-flattened-tokens-as-the-dispatch-unit` |
| Caption intent | Mapping between batch index, time index, flat index, and token vector. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Flattened tokens as the dispatch unit"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch06/eq-flattened-tokens-as-the-dispatch-unit.tex` |
| Include command | `\input{equations/ch06/eq-flattened-tokens-as-the-dispatch-unit}` |
| Label | `eq:ch06-flattened-tokens-as-the-dispatch-unit` |
| Equation intent | n = b*T + t maps (b,t) to flat token index. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch06/lst-flattened-tokens-as-the-dispatch-unit.tex` |
| Include command | `\input{listings/ch06/lst-flattened-tokens-as-the-dispatch-unit}` |
| Label | `lst:ch06-flattened-tokens-as-the-dispatch-unit` |
| Listing intent | flatten_tokens and unflatten_tokens helper functions. |
...
2. Add only the minimal new object or function needed for **Flattened tokens as the dispatch unit**.
3. Write a CPU smoke test using the mini-example: Flatten B=2, T=3, D=4 into N=6 token rows and recover B,T,D afterward.
...
**Expected code deliverable:** flatten_tokens and unflatten_tokens helper functions.
