---
name: moe-ch05-s05-numerical-stability-in-top-k-gates
description: use when developing section 5.5, numerical stability in top-k gates, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 5.5 - Numerical stability in top-k gates
...
Develop only Section 5.5 of *MoE Models from Scratch*: **Numerical stability in top-k gates**.

- Chapter: 5. Top-2 routing and weighted expert combination
- Target section file: `latex_book_skeleton/chapters/ch05/sections/05-numerical-stability-in-top-k-gates.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch05/fig-numerical-stability-in-top-k-gates.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch05/tab-numerical-stability-in-top-k-gates.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch05/eq-numerical-stability-in-top-k-gates.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch05/lst-numerical-stability-in-top-k-gates.tex`
- Required diagnostic: Unit tests for finite gates and gradients under small scores.
...
```text
Develop Section 5.5, "Numerical stability in top-k gates", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 5.5 - Numerical stability in top-k gates

Develop only Chapter 5.5, **Numerical stability in top-k gates**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch05/05-numerical-stability-in-top-k-gates.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch05/sections/05-numerical-stability-in-top-k-gates.tex`
