---
name: moe-ch03-s04-naive-token-by-token-dispatch
description: use when developing section 3.4, naive token-by-token dispatch, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 3.4 - Naive token-by-token dispatch
...
Develop only Section 3.4 of *MoE Models from Scratch*: **Naive token-by-token dispatch**.

- Chapter: 3. Experts, routers, and the first MoE layer
- Target section file: `latex_book_skeleton/chapters/ch03/sections/04-naive-token-by-token-dispatch.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch03/fig-naive-token-by-token-dispatch.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch03/tab-naive-token-by-token-dispatch.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch03/eq-naive-token-by-token-dispatch.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch03/lst-naive-token-by-token-dispatch.tex`
- Required diagnostic: Compare output shape to dense FFN output and inspect gates per token.
...
```text
Develop Section 3.4, "Naive token-by-token dispatch", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 3.4 - Naive token-by-token dispatch

Develop only Chapter 3.4, **Naive token-by-token dispatch**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch03/04-naive-token-by-token-dispatch.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch03/sections/04-naive-token-by-token-dispatch.tex`
...
# Detailed section development brief: Chapter 3.4 - Naive token-by-token dispatch
...
| Chapter | 3. Experts, routers, and the first MoE layer |
| Section | 3.4. Naive token-by-token dispatch |
| Section label slug | `ch03-naive-token-by-token-dispatch` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch03/sections/04-naive-token-by-token-dispatch.tex` |
| Planned section include | `\input{chapters/ch03/sections/04-naive-token-by-token-dispatch}` |
| Previous handoff | Chapter 3.3 - Top-k selection without dispatch |
| Next handoff | Chapter 3.5 - Shape and gradient checks |
...
**Mini-example to anchor the section:** For each token and selected expert, call the expert and add gate-weighted output.

## 3. Role in the chapter and book

**Chapter role:** Introduce experts and routers as explicit replacements for a dense FFN, first with naive readable code.

**Reader enters knowing:** Reader has a dense FFN and can train a tiny model.

**Reader leaves with:** Reader can compute router scores, select experts, dispatch tokens naively, and verify shapes/gradients.

**Chapter-level invariant:** The first MoE layer must accept x with shape (B, T, D) and return y with the same shape plus routing metadata.

**This section's bridge sentence:** The naive layer works, but the router choices are still fragile; top-1 routing exposes this clearly.
...
1. **Opening paragraph.** State the concrete problem: Implement dispatch with loops first so the control flow is unmistakable. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: For each token and selected expert, call the expert and add gate-weighted output.
3. **Visual explanation.** Insert `\input{figures/ch03/fig-naive-token-by-token-dispatch}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch03/eq-naive-token-by-token-dispatch}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch03/lst-naive-token-by-token-dispatch}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch03/tab-naive-token-by-token-dispatch}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch03/fig-naive-token-by-token-dispatch.tex` |
| Include command | `\input{figures/ch03/fig-naive-token-by-token-dispatch}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch03-naive-token-by-token-dispatch` |
| Caption intent | Loop-based dispatch arrows from token rows to selected expert calls. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Loop-based dispatch arrows from token rows to selected expert calls.

**Visual construction notes:** Use router cyan and expert purple heavily. Show matrices with N tokens by E experts and preserve token identity in diagrams. Use semantic TikZ styles from `styles/tikzstyles.tex`. Do not use raw hex colors in the diagram body. If this is a generated PDF plot, save the plotted PDF under `figures/generated/ch03/` and keep the wrapper responsible only for `\includegraphics`, caption, and label.

### Table placeholder

| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch03/tab-naive-token-by-token-dispatch.tex` |
| Include command | `\input{tables/ch03/tab-naive-token-by-token-dispatch}` |
| Label | `tab:ch03-naive-token-by-token-dispatch` |
| Caption intent | Naive dispatch variables and their role in the loop. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Naive token-by-token dispatch"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch03/eq-naive-token-by-token-dispatch.tex` |
| Include command | `\input{equations/ch03/eq-naive-token-by-token-dispatch}` |
| Label | `eq:ch03-naive-token-by-token-dispatch` |
| Equation intent | routed_out_t = sum_{j=1}^K gate_{tj} expert_{idx_{tj}}(x_t). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch03/lst-naive-token-by-token-dispatch.tex` |
| Include command | `\input{listings/ch03/lst-naive-token-by-token-dispatch}` |
| Label | `lst:ch03-naive-token-by-token-dispatch` |
| Listing intent | NaiveMoELayer forward pass using explicit token and expert loops. |

**Code objects to develop or reference:**
- `NaiveMoELayer.forward`
- `per-token expert loop`
...
**Primary code location:** `ch03/01_main-chapter-code/ with Router, ExpertMLP, and NaiveMoELayer prototypes.`

1. Start from the previous section's tested code, not from a fresh implementation.
2. Add only the minimal new object or function needed for **Naive token-by-token dispatch**.
3. Write a CPU smoke test using the mini-example: For each token and selected expert, call the expert and add gate-weighted output.
4. Preserve the chapter-level invariant: The first MoE layer must accept x with shape (B, T, D) and return y with the same shape plus routing metadata.
...
**Expected code deliverable:** NaiveMoELayer forward pass using explicit token and expert loops.
