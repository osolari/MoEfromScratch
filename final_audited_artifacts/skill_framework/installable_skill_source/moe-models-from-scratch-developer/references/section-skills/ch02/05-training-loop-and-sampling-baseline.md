---
name: moe-ch02-s05-training-loop-and-sampling-baseline
description: use when developing section 2.5, training loop and sampling baseline, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 2.5 - Training loop and sampling baseline
...
Develop only Section 2.5 of *MoE Models from Scratch*: **Training loop and sampling baseline**.

- Chapter: 2. A dense Transformer baseline from scratch
- Target section file: `latex_book_skeleton/chapters/ch02/sections/05-training-loop-and-sampling-baseline.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch02/fig-training-loop-and-sampling-baseline.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch02/tab-training-loop-and-sampling-baseline.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch02/eq-training-loop-and-sampling-baseline.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch02/lst-training-loop-and-sampling-baseline.tex`
- Required diagnostic: Track train and validation loss plus samples at fixed intervals.
...
```text
Develop Section 2.5, "Training loop and sampling baseline", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 2.5 - Training loop and sampling baseline

Develop only Chapter 2.5, **Training loop and sampling baseline**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch02/05-training-loop-and-sampling-baseline.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch02/sections/05-training-loop-and-sampling-baseline.tex`
...
# Detailed section development brief: Chapter 2.5 - Training loop and sampling baseline
...
| Chapter | 2. A dense Transformer baseline from scratch |
| Section | 2.5. Training loop and sampling baseline |
| Section label slug | `ch02-training-loop-and-sampling-baseline` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch02/sections/05-training-loop-and-sampling-baseline.tex` |
| Planned section include | `\input{chapters/ch02/sections/05-training-loop-and-sampling-baseline}` |
| Previous handoff | Chapter 2.4 - Dense feed-forward block as the replacement target |
| Next handoff | Chapter 2.6 - Baseline diagnostics and comparison slots |
...
**Mini-example to anchor the section:** Run a smoke train for a few iterations and sample a short continuation.
...
**This section's bridge sentence:** Now that the dense baseline works, the next chapter isolates the expert and router pieces.
...
1. **Opening paragraph.** State the concrete problem: Train the dense model long enough to create a trustworthy comparison anchor. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Run a smoke train for a few iterations and sample a short continuation.
3. **Visual explanation.** Insert `\input{figures/ch02/fig-training-loop-and-sampling-baseline}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch02/eq-training-loop-and-sampling-baseline}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch02/lst-training-loop-and-sampling-baseline}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch02/tab-training-loop-and-sampling-baseline}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch02/fig-training-loop-and-sampling-baseline.tex` |
| Include command | `\input{figures/ch02/fig-training-loop-and-sampling-baseline}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch02-training-loop-and-sampling-baseline` |
| Caption intent | Training loop pipeline: batch, forward, loss, backward, optimizer, sample. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Training loop pipeline: batch, forward, loss, backward, optimizer, sample.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch02/tab-training-loop-and-sampling-baseline.tex` |
| Include command | `\input{tables/ch02/tab-training-loop-and-sampling-baseline}` |
| Label | `tab:ch02-training-loop-and-sampling-baseline` |
| Caption intent | Baseline configuration fields and default smoke-test values. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Training loop and sampling baseline"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch02/eq-training-loop-and-sampling-baseline.tex` |
| Include command | `\input{equations/ch02/eq-training-loop-and-sampling-baseline}` |
| Label | `eq:ch02-training-loop-and-sampling-baseline` |
| Equation intent | Cross-entropy over shifted logits and targets. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch02/lst-training-loop-and-sampling-baseline.tex` |
| Include command | `\input{listings/ch02/lst-training-loop-and-sampling-baseline}` |
| Label | `lst:ch02-training-loop-and-sampling-baseline` |
| Listing intent | Train step with loss, backward pass, clipping, optimizer step, and logging. |
...
2. Add only the minimal new object or function needed for **Training loop and sampling baseline**.
3. Write a CPU smoke test using the mini-example: Run a smoke train for a few iterations and sample a short continuation.
...
**Expected code deliverable:** Train step with loss, backward pass, clipping, optimizer step, and logging.
