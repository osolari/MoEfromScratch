---
name: moe-ch08-s04-combining-shared-and-routed-outputs
description: use when developing section 8.4, combining shared and routed outputs, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 8.4 - Combining shared and routed outputs
...
Develop only Section 8.4 of *MoE Models from Scratch*: **Combining shared and routed outputs**.

- Chapter: 8. Shared experts and fine-grained routed experts
- Target section file: `latex_book_skeleton/chapters/ch08/sections/04-combining-shared-and-routed-outputs.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch08/fig-combining-shared-and-routed-outputs.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch08/tab-combining-shared-and-routed-outputs.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch08/eq-combining-shared-and-routed-outputs.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch08/lst-combining-shared-and-routed-outputs.tex`
- Required diagnostic: Ablate shared_out to zero and routed_out to zero to confirm both paths affect output.
...
```text
Develop Section 8.4, "Combining shared and routed outputs", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 8.4 - Combining shared and routed outputs

Develop only Chapter 8.4, **Combining shared and routed outputs**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch08/04-combining-shared-and-routed-outputs.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch08/sections/04-combining-shared-and-routed-outputs.tex`
...
# Detailed section development brief: Chapter 8.4 - Combining shared and routed outputs
...
| Chapter | 8. Shared experts and fine-grained routed experts |
| Section | 8.4. Combining shared and routed outputs |
| Section label slug | `ch08-combining-shared-and-routed-outputs` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch08/sections/04-combining-shared-and-routed-outputs.tex` |
| Planned section include | `\input{chapters/ch08/sections/04-combining-shared-and-routed-outputs}` |
| Previous handoff | Chapter 8.3 - The MiniDeepSeekMoE layer contract |
| Next handoff | Chapter 8.5 - Specialization diagnostics |
...
**Mini-example to anchor the section:** Trace one token through shared expert s0 and routed experts e2/e7.
...
**This section's bridge sentence:** With two expert paths, diagnostics should separate shared and routed behavior.
...
1. **Opening paragraph.** State the concrete problem: Implement the forward pass that sums shared expert output and weighted routed output. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Trace one token through shared expert s0 and routed experts e2/e7.
3. **Visual explanation.** Insert `\input{figures/ch08/fig-combining-shared-and-routed-outputs}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch08/eq-combining-shared-and-routed-outputs}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch08/lst-combining-shared-and-routed-outputs}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch08/tab-combining-shared-and-routed-outputs}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch08/fig-combining-shared-and-routed-outputs.tex` |
| Include command | `\input{figures/ch08/fig-combining-shared-and-routed-outputs}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch08-combining-shared-and-routed-outputs` |
| Caption intent | Shared output and routed weighted output merged before projection back to the decoder block. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Shared output and routed weighted output merged before projection back to the decoder block.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch08/tab-combining-shared-and-routed-outputs.tex` |
| Include command | `\input{tables/ch08/tab-combining-shared-and-routed-outputs}` |
| Label | `tab:ch08-combining-shared-and-routed-outputs` |
| Caption intent | Output components: shared_out, routed_out, residual, and final block output. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Combining shared and routed outputs"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch08/eq-combining-shared-and-routed-outputs.tex` |
| Include command | `\input{equations/ch08/eq-combining-shared-and-routed-outputs}` |
| Label | `eq:ch08-combining-shared-and-routed-outputs` |
| Equation intent | moe_out_t = shared_out_t + sum_j gate_{tj} routed_expert_{idx_{tj}}(x_t). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch08/lst-combining-shared-and-routed-outputs.tex` |
| Include command | `\input{listings/ch08/lst-combining-shared-and-routed-outputs}` |
| Label | `lst:ch08-combining-shared-and-routed-outputs` |
| Listing intent | Forward pass for shared+routed MoE layer. |
...
2. Add only the minimal new object or function needed for **Combining shared and routed outputs**.
3. Write a CPU smoke test using the mini-example: Trace one token through shared expert s0 and routed experts e2/e7.
...
**Expected code deliverable:** Forward pass for shared+routed MoE layer.
