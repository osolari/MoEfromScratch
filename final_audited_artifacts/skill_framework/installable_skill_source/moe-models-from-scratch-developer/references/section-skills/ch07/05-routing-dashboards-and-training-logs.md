---
name: moe-ch07-s05-routing-dashboards-and-training-logs
description: use when developing section 7.5, routing dashboards and training logs, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 7.5 - Routing dashboards and training logs
...
Develop only Section 7.5 of *MoE Models from Scratch*: **Routing dashboards and training logs**.

- Chapter: 7. Router training and load-balancing losses
- Target section file: `latex_book_skeleton/chapters/ch07/sections/05-routing-dashboards-and-training-logs.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch07/fig-routing-dashboards-and-training-logs.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch07/tab-routing-dashboards-and-training-logs.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch07/eq-routing-dashboards-and-training-logs.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch07/lst-routing-dashboards-and-training-logs.tex`
- Required diagnostic: Generate a dashboard PDF or image after a short smoke run.
...
```text
Develop Section 7.5, "Routing dashboards and training logs", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 7.5 - Routing dashboards and training logs

Develop only Chapter 7.5, **Routing dashboards and training logs**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch07/05-routing-dashboards-and-training-logs.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch07/sections/05-routing-dashboards-and-training-logs.tex`
...
# Detailed section development brief: Chapter 7.5 - Routing dashboards and training logs
...
| Chapter | 7. Router training and load-balancing losses |
| Section | 7.5. Routing dashboards and training logs |
| Section label slug | `ch07-routing-dashboards-and-training-logs` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch07/sections/05-routing-dashboards-and-training-logs.tex` |
| Planned section include | `\input{chapters/ch07/sections/05-routing-dashboards-and-training-logs}` |
| Previous handoff | Chapter 7.4 - Router score scale and z-loss |
| Next handoff | Chapter 7.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Log one training batch with load histogram, gate entropy, drop count, and loss terms.
...
**This section's bridge sentence:** The next chapter changes the expert architecture itself by adding shared and fine-grained routed experts.
...
1. **Opening paragraph.** State the concrete problem: Create the standard diagnostic views used for every later MoE experiment. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Log one training batch with load histogram, gate entropy, drop count, and loss terms.
3. **Visual explanation.** Insert `\input{figures/ch07/fig-routing-dashboards-and-training-logs}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch07/eq-routing-dashboards-and-training-logs}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch07/lst-routing-dashboards-and-training-logs}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch07/tab-routing-dashboards-and-training-logs}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch07/fig-routing-dashboards-and-training-logs.tex` |
| Include command | `\input{figures/ch07/fig-routing-dashboards-and-training-logs}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch07-routing-dashboards-and-training-logs` |
| Caption intent | Dashboard layout with loss curve, expert load histogram, gate entropy, and drop rate. |
...
**Figure content plan:** Dashboard layout with loss curve, expert load histogram, gate entropy, and drop rate.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch07/tab-routing-dashboards-and-training-logs.tex` |
| Include command | `\input{tables/ch07/tab-routing-dashboards-and-training-logs}` |
| Label | `tab:ch07-routing-dashboards-and-training-logs` |
| Caption intent | Metric names, tensor source, frequency, and expected range. |
| Required columns | Metric or run | What it measures | Expected healthy pattern | Warning pattern | Where logged |

**Planned table rows:**
- row 1: define the primary object for "Routing dashboards and training logs"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch07/eq-routing-dashboards-and-training-logs.tex` |
| Include command | `\input{equations/ch07/eq-routing-dashboards-and-training-logs}` |
| Label | `eq:ch07-routing-dashboards-and-training-logs` |
| Equation intent | Routing entropy over normalized selected gates. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch07/lst-routing-dashboards-and-training-logs.tex` |
| Include command | `\input{listings/ch07/lst-routing-dashboards-and-training-logs}` |
| Label | `lst:ch07-routing-dashboards-and-training-logs` |
| Listing intent | collect_router_metrics and plot_router_dashboard helpers. |
...
2. Add only the minimal new object or function needed for **Routing dashboards and training logs**.
3. Write a CPU smoke test using the mini-example: Log one training batch with load histogram, gate entropy, drop count, and loss terms.
...
**Expected code deliverable:** collect_router_metrics and plot_router_dashboard helpers.
