---
name: moe-ch07-s03-integrating-auxiliary-loss-into-training
description: use when developing section 7.3, integrating auxiliary loss into training, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 7.3 - Integrating auxiliary loss into training
...
Develop only Section 7.3 of *MoE Models from Scratch*: **Integrating auxiliary loss into training**.

- Chapter: 7. Router training and load-balancing losses
- Target section file: `latex_book_skeleton/chapters/ch07/sections/03-integrating-auxiliary-loss-into-training.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch07/fig-integrating-auxiliary-loss-into-training.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch07/tab-integrating-auxiliary-loss-into-training.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch07/eq-integrating-auxiliary-loss-into-training.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch07/lst-integrating-auxiliary-loss-into-training.tex`
- Required diagnostic: Log individual losses separately so balancing cannot hide LM degradation.
...
```text
Develop Section 7.3, "Integrating auxiliary loss into training", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 7.3 - Integrating auxiliary loss into training

Develop only Chapter 7.3, **Integrating auxiliary loss into training**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch07/03-integrating-auxiliary-loss-into-training.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch07/sections/03-integrating-auxiliary-loss-into-training.tex`
...
# Detailed section development brief: Chapter 7.3 - Integrating auxiliary loss into training
...
| Chapter | 7. Router training and load-balancing losses |
| Section | 7.3. Integrating auxiliary loss into training |
| Section label slug | `ch07-integrating-auxiliary-loss-into-training` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch07/sections/03-integrating-auxiliary-loss-into-training.tex` |
| Planned section include | `\input{chapters/ch07/sections/03-integrating-auxiliary-loss-into-training}` |
| Previous handoff | Chapter 7.2 - Auxiliary load-balancing loss |
| Next handoff | Chapter 7.4 - Router score scale and z-loss |
...
**Mini-example to anchor the section:** Return lm_loss, aux_loss, total_loss, and router_metrics from one forward pass.

## 3. Role in the chapter and book

**Chapter role:** Show that routers are trainable systems with failure modes and balancing objectives, not just argmax functions.

**Reader enters knowing:** Reader can route and dispatch tokens but has seen imbalance and overflow.

**Reader leaves with:** Reader can add an auxiliary load-balancing loss, z-loss, and routing dashboards to the training loop.

**Chapter-level invariant:** Balancing losses influence router training but should not change the model forward interface for inference.

**This section's bridge sentence:** Load balancing is one router signal; score scale can also create instability.
...
1. **Opening paragraph.** State the concrete problem: Add routing losses to the model output dictionary without hiding the main language-model loss. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Return lm_loss, aux_loss, total_loss, and router_metrics from one forward pass.
3. **Visual explanation.** Insert `\input{figures/ch07/fig-integrating-auxiliary-loss-into-training}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch07/eq-integrating-auxiliary-loss-into-training}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch07/lst-integrating-auxiliary-loss-into-training}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch07/tab-integrating-auxiliary-loss-into-training}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch07/fig-integrating-auxiliary-loss-into-training.tex` |
| Include command | `\input{figures/ch07/fig-integrating-auxiliary-loss-into-training}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch07-integrating-auxiliary-loss-into-training` |
| Caption intent | Loss composition diagram from logits and router statistics to total loss. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Loss composition diagram from logits and router statistics to total loss.

**Visual construction notes:** Use diagnostic plots as PDF placeholders when the artifact is meant to come from code. Use red carefully for imbalance. Use semantic TikZ styles from `styles/tikzstyles.tex`. Do not use raw hex colors in the diagram body. If this is a generated PDF plot, save the plotted PDF under `figures/generated/ch07/` and keep the wrapper responsible only for `\includegraphics`, caption, and label.

### Table placeholder

| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch07/tab-integrating-auxiliary-loss-into-training.tex` |
| Include command | `\input{tables/ch07/tab-integrating-auxiliary-loss-into-training}` |
| Label | `tab:ch07-integrating-auxiliary-loss-into-training` |
| Caption intent | Loss terms, coefficients, default values, and logging names. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Integrating auxiliary loss into training"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch07/eq-integrating-auxiliary-loss-into-training.tex` |
| Include command | `\input{equations/ch07/eq-integrating-auxiliary-loss-into-training}` |
| Label | `eq:ch07-integrating-auxiliary-loss-into-training` |
| Equation intent | total_loss = lm_loss + lambda_aux * aux_loss. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch07/lst-integrating-auxiliary-loss-into-training.tex` |
| Include command | `\input{listings/ch07/lst-integrating-auxiliary-loss-into-training}` |
| Label | `lst:ch07-integrating-auxiliary-loss-into-training` |
| Listing intent | Training step that combines lm_loss + aux_coef * aux_loss. |

**Code objects to develop or reference:**
- `load_balancing_loss`
- `router probability statistics`
...
**Primary code location:** `ch07/01_main-chapter-code/ with losses.py, training integration, and log/dashboard generation.`

1. Start from the previous section's tested code, not from a fresh implementation.
2. Add only the minimal new object or function needed for **Integrating auxiliary loss into training**.
3. Write a CPU smoke test using the mini-example: Return lm_loss, aux_loss, total_loss, and router_metrics from one forward pass.
4. Preserve the chapter-level invariant: Balancing losses influence router training but should not change the model forward interface for inference.
...
**Expected code deliverable:** Training step that combines lm_loss + aux_coef * aux_loss.
