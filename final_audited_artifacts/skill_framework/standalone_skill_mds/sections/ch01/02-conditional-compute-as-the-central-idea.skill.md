---
name: moe-ch01-s02-conditional-compute-as-the-central-idea
description: use when developing section 1.2, conditional compute as the central idea, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 1.2 - Conditional compute as the central idea
...
Develop only Section 1.2 of *MoE Models from Scratch*: **Conditional compute as the central idea**.

- Chapter: 1. From dense compute to conditional compute
- Target section file: `latex_book_skeleton/chapters/ch01/sections/02-conditional-compute-as-the-central-idea.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch01/fig-conditional-compute-as-the-central-idea.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch01/tab-conditional-compute-as-the-central-idea.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch01/eq-conditional-compute-as-the-central-idea.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch01/lst-conditional-compute-as-the-central-idea.tex`
- Required diagnostic: Compute the active fraction for E=4, K=2 and E=16, K=2.
...
```text
Develop Section 1.2, "Conditional compute as the central idea", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 1.2 - Conditional compute as the central idea

Develop only Chapter 1.2, **Conditional compute as the central idea**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch01/02-conditional-compute-as-the-central-idea.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch01/sections/02-conditional-compute-as-the-central-idea.tex`
...
# Detailed section development brief: Chapter 1.2 - Conditional compute as the central idea
...
| Chapter | 1. From dense compute to conditional compute |
| Section | 1.2. Conditional compute as the central idea |
| Section label slug | `ch01-conditional-compute-as-the-central-idea` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch01/sections/02-conditional-compute-as-the-central-idea.tex` |
| Planned section include | `\input{chapters/ch01/sections/02-conditional-compute-as-the-central-idea}` |
| Previous handoff | Chapter 1.1 - The dense FFN bottleneck |
| Next handoff | Chapter 1.3 - A tiny routing story |
...
**Mini-example to anchor the section:** Route four tokens to two of four available experts and compare activated parameters against total parameters.

## 3. Role in the chapter and book

**Chapter role:** Motivate MoE by making the dense FFN bottleneck visible before any router is introduced.

**Reader enters knowing:** Reader knows basic decoder blocks at a high level but has not yet implemented a model in this book.

**Reader leaves with:** Reader understands the build ladder and can explain why conditional compute targets the FFN path.

**Chapter-level invariant:** The decoder interface remains x -> y with shape (B, T, D) even when compute becomes conditional.

**This section's bridge sentence:** Selective compute creates a new component: the router, which needs its own tensor contract.
...
1. **Opening paragraph.** State the concrete problem: Introduce MoE as selective FFN computation rather than a mysterious new model family. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Route four tokens to two of four available experts and compare activated parameters against total parameters.
3. **Visual explanation.** Insert `\input{figures/ch01/fig-conditional-compute-as-the-central-idea}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch01/eq-conditional-compute-as-the-central-idea}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch01/lst-conditional-compute-as-the-central-idea}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch01/tab-conditional-compute-as-the-central-idea}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch01/fig-conditional-compute-as-the-central-idea.tex` |
| Include command | `\input{figures/ch01/fig-conditional-compute-as-the-central-idea}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch01-conditional-compute-as-the-central-idea` |
| Caption intent | Tokens split through a router into a sparse subset of experts, then recombine. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Tokens split through a router into a sparse subset of experts, then recombine.

**Visual construction notes:** Use simple blocks and arrows. Prefer token dots, a highlighted dense FFN path, and a roadmap ladder. No heatmaps yet. Use semantic TikZ styles from `styles/tikzstyles.tex`. Do not use raw hex colors in the diagram body. If this is a generated PDF plot, save the plotted PDF under `figures/generated/ch01/` and keep the wrapper responsible only for `\includegraphics`, caption, and label.

### Table placeholder

| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch01/tab-conditional-compute-as-the-central-idea.tex` |
| Include command | `\input{tables/ch01/tab-conditional-compute-as-the-central-idea}` |
| Label | `tab:ch01-conditional-compute-as-the-central-idea` |
| Caption intent | Dense versus conditional-compute vocabulary: FFN, expert, router, active parameters, total parameters. |
| Required columns | Quantity | Dense baseline | MoE path | Caveat | Measurement |

**Planned table rows:**
- row 1: define the primary object for "Conditional compute as the central idea"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch01/eq-conditional-compute-as-the-central-idea.tex` |
| Include command | `\input{equations/ch01/eq-conditional-compute-as-the-central-idea}` |
| Label | `eq:ch01-conditional-compute-as-the-central-idea` |
| Equation intent | Activated-parameter view: total expert bank versus top-k expert use per token. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch01/lst-conditional-compute-as-the-central-idea.tex` |
| Include command | `\input{listings/ch01/lst-conditional-compute-as-the-central-idea}` |
| Label | `lst:ch01-conditional-compute-as-the-central-idea` |
| Listing intent | Pseudocode that chooses expert IDs for tokens and reports active expert count. |

**Code objects to develop or reference:**
- `cost_estimate.py or notebook cell`
- `tiny routing tensor example`
- `roadmap/config dictionary`
...
**Primary code location:** `ch01/01_main-chapter-code/ with small notebooks or scripts for cost estimates and hand-built routing tensors.`

1. Start from the previous section's tested code, not from a fresh implementation.
2. Add only the minimal new object or function needed for **Conditional compute as the central idea**.
3. Write a CPU smoke test using the mini-example: Route four tokens to two of four available experts and compare activated parameters against total parameters.
4. Preserve the chapter-level invariant: The decoder interface remains x -> y with shape (B, T, D) even when compute becomes conditional.
...
**Expected code deliverable:** Pseudocode that chooses expert IDs for tokens and reports active expert count.
