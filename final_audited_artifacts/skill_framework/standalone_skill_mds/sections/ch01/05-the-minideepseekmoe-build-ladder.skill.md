---
name: moe-ch01-s05-the-minideepseekmoe-build-ladder
description: use when developing section 1.5, the minideepseekmoe build ladder, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 1.5 - The MiniDeepSeekMoE build ladder
...
Develop only Section 1.5 of *MoE Models from Scratch*: **The MiniDeepSeekMoE build ladder**.

- Chapter: 1. From dense compute to conditional compute
- Target section file: `latex_book_skeleton/chapters/ch01/sections/05-the-minideepseekmoe-build-ladder.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch01/fig-the-minideepseekmoe-build-ladder.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch01/tab-the-minideepseekmoe-build-ladder.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch01/eq-the-minideepseekmoe-build-ladder.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch01/lst-the-minideepseekmoe-build-ladder.tex`
- Required diagnostic: Milestone checklist that marks which diagnostics appear by each chapter.
...
```text
Develop Section 1.5, "The MiniDeepSeekMoE build ladder", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 1.5 - The MiniDeepSeekMoE build ladder

Develop only Chapter 1.5, **The MiniDeepSeekMoE build ladder**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch01/05-the-minideepseekmoe-build-ladder.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch01/sections/05-the-minideepseekmoe-build-ladder.tex`
...
# Detailed section development brief: Chapter 1.5 - The MiniDeepSeekMoE build ladder
...
| Chapter | 1. From dense compute to conditional compute |
| Section | 1.5. The MiniDeepSeekMoE build ladder |
| Section label slug | `ch01-the-minideepseekmoe-build-ladder` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch01/sections/05-the-minideepseekmoe-build-ladder.tex` |
| Planned section include | `\input{chapters/ch01/sections/05-the-minideepseekmoe-build-ladder}` |
| Previous handoff | Chapter 1.4 - What from scratch means for this book |
| Next handoff | Chapter 1.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Use the smoke-test configuration with two layers, four routed experts, and one shared expert.
...
**This section's bridge sentence:** The first build step is a dense Transformer baseline that gives the MoE layer somewhere to live.
...
1. **Opening paragraph.** State the concrete problem: Preview the book path from dense baseline to shared experts, fine-grained experts, and router-bias balancing. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use the smoke-test configuration with two layers, four routed experts, and one shared expert.
3. **Visual explanation.** Insert `\input{figures/ch01/fig-the-minideepseekmoe-build-ladder}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch01/eq-the-minideepseekmoe-build-ladder}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch01/lst-the-minideepseekmoe-build-ladder}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch01/tab-the-minideepseekmoe-build-ladder}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch01/fig-the-minideepseekmoe-build-ladder.tex` |
| Include command | `\input{figures/ch01/fig-the-minideepseekmoe-build-ladder}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch01-the-minideepseekmoe-build-ladder` |
| Caption intent | Roadmap ladder: dense baseline, top-1, top-2, capacity, auxiliary loss, shared experts, bias balancing, full model. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Roadmap ladder: dense baseline, top-1, top-2, capacity, auxiliary loss, shared experts, bias balancing, full model.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch01/tab-the-minideepseekmoe-build-ladder.tex` |
| Include command | `\input{tables/ch01/tab-the-minideepseekmoe-build-ladder}` |
| Label | `tab:ch01-the-minideepseekmoe-build-ladder` |
| Caption intent | Chapter-by-chapter implementation milestones and expected code outputs. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "The MiniDeepSeekMoE build ladder"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch01/eq-the-minideepseekmoe-build-ladder.tex` |
| Include command | `\input{equations/ch01/eq-the-minideepseekmoe-build-ladder}` |
| Label | `eq:ch01-the-minideepseekmoe-build-ladder` |
| Equation intent | Final layer formula combining residual, shared expert output, and routed expert output. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch01/lst-the-minideepseekmoe-build-ladder.tex` |
| Include command | `\input{listings/ch01/lst-the-minideepseekmoe-build-ladder}` |
| Label | `lst:ch01-the-minideepseekmoe-build-ladder` |
| Listing intent | Configuration dictionary for MiniDeepSeekMoE-Smoke and MiniDeepSeekMoE-16x2. |
...
2. Add only the minimal new object or function needed for **The MiniDeepSeekMoE build ladder**.
3. Write a CPU smoke test using the mini-example: Use the smoke-test configuration with two layers, four routed experts, and one shared expert.
...
**Expected code deliverable:** Configuration dictionary for MiniDeepSeekMoE-Smoke and MiniDeepSeekMoE-16x2.
