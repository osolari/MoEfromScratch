---
name: moe-ch09-s02-router-bias-as-non-trainable-state
description: use when developing section 9.2, router bias as non-trainable state, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 9.2 - Router bias as non-trainable state
...
Develop only Section 9.2 of *MoE Models from Scratch*: **Router bias as non-trainable state**.

- Chapter: 9. Auxiliary-loss-free load balancing
- Target section file: `latex_book_skeleton/chapters/ch09/sections/02-router-bias-as-non-trainable-state.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch09/fig-router-bias-as-non-trainable-state.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch09/tab-router-bias-as-non-trainable-state.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch09/eq-router-bias-as-non-trainable-state.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch09/lst-router-bias-as-non-trainable-state.tex`
- Required diagnostic: Assert router_bias.requires_grad is false and optimizer does not update it.
...
```text
Develop Section 9.2, "Router bias as non-trainable state", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 9.2 - Router bias as non-trainable state
...
| Chapter | 9. Auxiliary-loss-free load balancing |
| Section | 9.2. Router bias as non-trainable state |
| Section label slug | `ch09-router-bias-as-non-trainable-state` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch09/sections/02-router-bias-as-non-trainable-state.tex` |
| Planned section include | `\input{chapters/ch09/sections/02-router-bias-as-non-trainable-state}` |
| Previous handoff | Chapter 9.1 - The limitation of auxiliary balancing |
| Next handoff | Chapter 9.3 - Biased selection and unbiased combine |
...
**Mini-example to anchor the section:** Use four experts with bias values that rise for underused experts and fall for overused experts.
...
**This section's bridge sentence:** The key implementation detail is that bias changes selection but not final gates.
...
1. **Opening paragraph.** State the concrete problem: Define router bias as a per-expert control signal updated outside backpropagation. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use four experts with bias values that rise for underused experts and fall for overused experts.
3. **Visual explanation.** Insert `\input{figures/ch09/fig-router-bias-as-non-trainable-state}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch09/eq-router-bias-as-non-trainable-state}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch09/lst-router-bias-as-non-trainable-state}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch09/tab-router-bias-as-non-trainable-state}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch09/fig-router-bias-as-non-trainable-state.tex` |
| Include command | `\input{figures/ch09/fig-router-bias-as-non-trainable-state}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch09-router-bias-as-non-trainable-state` |
| Caption intent | Bias vector nudging the score table before top-k selection. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Bias vector nudging the score table before top-k selection.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch09/tab-router-bias-as-non-trainable-state.tex` |
| Include command | `\input{tables/ch09/tab-router-bias-as-non-trainable-state}` |
| Label | `tab:ch09-router-bias-as-non-trainable-state` |
| Caption intent | Router bias properties: shape, initialization, update timing, gradient status, checkpoint behavior. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Router bias as non-trainable state"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch09/eq-router-bias-as-non-trainable-state.tex` |
| Include command | `\input{equations/ch09/eq-router-bias-as-non-trainable-state}` |
| Label | `eq:ch09-router-bias-as-non-trainable-state` |
| Equation intent | selection_score = score + router_bias. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch09/lst-router-bias-as-non-trainable-state.tex` |
| Include command | `\input{listings/ch09/lst-router-bias-as-non-trainable-state}` |
| Label | `lst:ch09-router-bias-as-non-trainable-state` |
| Listing intent | Register router_bias as a buffer and exclude it from optimizer gradients. |

**Code objects to develop or reference:**
- `RouterBiasState`
- `non-trainable buffer registration`
...
2. Add only the minimal new object or function needed for **Router bias as non-trainable state**.
3. Write a CPU smoke test using the mini-example: Use four experts with bias values that rise for underused experts and fall for overused experts.
...
**Expected code deliverable:** Register router_bias as a buffer and exclude it from optimizer gradients.
