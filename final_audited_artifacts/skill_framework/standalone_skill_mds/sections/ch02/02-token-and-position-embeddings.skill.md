---
name: moe-ch02-s02-token-and-position-embeddings
description: use when developing section 2.2, token and position embeddings, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 2.2 - Token and position embeddings
...
Develop only Section 2.2 of *MoE Models from Scratch*: **Token and position embeddings**.

- Chapter: 2. A dense Transformer baseline from scratch
- Target section file: `latex_book_skeleton/chapters/ch02/sections/02-token-and-position-embeddings.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch02/fig-token-and-position-embeddings.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch02/tab-token-and-position-embeddings.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch02/eq-token-and-position-embeddings.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch02/lst-token-and-position-embeddings.tex`
- Required diagnostic: Assert output shape and confirm gradients flow to both embedding tables.
...
```text
Develop Section 2.2, "Token and position embeddings", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 2.2 - Token and position embeddings

Develop only Chapter 2.2, **Token and position embeddings**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch02/02-token-and-position-embeddings.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch02/sections/02-token-and-position-embeddings.tex`
