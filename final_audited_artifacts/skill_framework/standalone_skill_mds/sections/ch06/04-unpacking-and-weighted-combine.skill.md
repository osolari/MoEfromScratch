---
name: moe-ch06-s04-unpacking-and-weighted-combine
description: use when developing section 6.4, unpacking and weighted combine, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 6.4 - Unpacking and weighted combine
...
Develop only Section 6.4 of *MoE Models from Scratch*: **Unpacking and weighted combine**.

- Chapter: 6. Vectorized dispatch, capacity, and routing efficiency
- Target section file: `latex_book_skeleton/chapters/ch06/sections/04-unpacking-and-weighted-combine.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch06/fig-unpacking-and-weighted-combine.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch06/tab-unpacking-and-weighted-combine.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch06/eq-unpacking-and-weighted-combine.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch06/lst-unpacking-and-weighted-combine.tex`
- Required diagnostic: Compare vectorized output to naive output when capacity is large enough for no overflow.
...
```text
Develop Section 6.4, "Unpacking and weighted combine", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 6.4 - Unpacking and weighted combine

Develop only Chapter 6.4, **Unpacking and weighted combine**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch06/04-unpacking-and-weighted-combine.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch06/sections/04-unpacking-and-weighted-combine.tex`
...
# Detailed section development brief: Chapter 6.4 - Unpacking and weighted combine
...
| Chapter | 6. Vectorized dispatch, capacity, and routing efficiency |
| Section | 6.4. Unpacking and weighted combine |
| Section label slug | `ch06-unpacking-and-weighted-combine` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch06/sections/04-unpacking-and-weighted-combine.tex` |
| Planned section include | `\input{chapters/ch06/sections/04-unpacking-and-weighted-combine}` |
| Previous handoff | Chapter 6.3 - Capacity factors and overflow policy |
| Next handoff | Chapter 6.5 - Equivalence tests against the naive layer |
...
**Mini-example to anchor the section:** One token receives two routes; one overflow route is skipped while the accepted route contributes.
...
**This section's bridge sentence:** The implementation is faster, but correctness tests must guard against silent routing mistakes.
...
1. **Opening paragraph.** State the concrete problem: Scatter expert-batch outputs back to token rows and apply route weights correctly. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: One token receives two routes; one overflow route is skipped while the accepted route contributes.
3. **Visual explanation.** Insert `\input{figures/ch06/fig-unpacking-and-weighted-combine}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch06/eq-unpacking-and-weighted-combine}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch06/lst-unpacking-and-weighted-combine}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch06/tab-unpacking-and-weighted-combine}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch06/fig-unpacking-and-weighted-combine.tex` |
| Include command | `\input{figures/ch06/fig-unpacking-and-weighted-combine}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch06-unpacking-and-weighted-combine` |
| Caption intent | Expert outputs unpacked through token and slot indices into routed_out. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Expert outputs unpacked through token and slot indices into routed_out.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch06/tab-unpacking-and-weighted-combine.tex` |
| Include command | `\input{tables/ch06/tab-unpacking-and-weighted-combine}` |
| Label | `tab:ch06-unpacking-and-weighted-combine` |
| Caption intent | Accepted route table: token, expert, slot, gate, and output contribution. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Unpacking and weighted combine"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch06/eq-unpacking-and-weighted-combine.tex` |
| Include command | `\input{equations/ch06/eq-unpacking-and-weighted-combine}` |
| Label | `eq:ch06-unpacking-and-weighted-combine` |
| Equation intent | routed_out[token] += gate * expert_out[expert, slot]. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch06/lst-unpacking-and-weighted-combine.tex` |
| Include command | `\input{listings/ch06/lst-unpacking-and-weighted-combine}` |
| Label | `lst:ch06-unpacking-and-weighted-combine` |
| Listing intent | unpack_expert_outputs function using index_add or scatter_add. |

**Code objects to develop or reference:**
- `scatter_combine`
- `weighted output restoration`
...
2. Add only the minimal new object or function needed for **Unpacking and weighted combine**.
3. Write a CPU smoke test using the mini-example: One token receives two routes; one overflow route is skipped while the accepted route contributes.
...
**Expected code deliverable:** unpack_expert_outputs function using index_add or scatter_add.
