---
name: moe-ch06-s05-equivalence-tests-against-the-naive-layer
description: use when developing section 6.5, equivalence tests against the naive layer, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 6.5 - Equivalence tests against the naive layer
...
Develop only Section 6.5 of *MoE Models from Scratch*: **Equivalence tests against the naive layer**.

- Chapter: 6. Vectorized dispatch, capacity, and routing efficiency
- Target section file: `latex_book_skeleton/chapters/ch06/sections/05-equivalence-tests-against-the-naive-layer.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch06/fig-equivalence-tests-against-the-naive-layer.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch06/tab-equivalence-tests-against-the-naive-layer.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch06/eq-equivalence-tests-against-the-naive-layer.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch06/lst-equivalence-tests-against-the-naive-layer.tex`
- Required diagnostic: Fail if max_abs_diff exceeds tolerance in no-overflow settings.
...
```text
Develop Section 6.5, "Equivalence tests against the naive layer", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 6.5 - Equivalence tests against the naive layer

Develop only Chapter 6.5, **Equivalence tests against the naive layer**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch06/05-equivalence-tests-against-the-naive-layer.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch06/sections/05-equivalence-tests-against-the-naive-layer.tex`
...
# Detailed section development brief: Chapter 6.5 - Equivalence tests against the naive layer
...
| Chapter | 6. Vectorized dispatch, capacity, and routing efficiency |
| Section | 6.5. Equivalence tests against the naive layer |
| Section label slug | `ch06-equivalence-tests-against-the-naive-layer` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch06/sections/05-equivalence-tests-against-the-naive-layer.tex` |
| Planned section include | `\input{chapters/ch06/sections/05-equivalence-tests-against-the-naive-layer}` |
| Previous handoff | Chapter 6.4 - Unpacking and weighted combine |
| Next handoff | Chapter 6.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Use fixed router scores and identical expert weights for both layers.

## 3. Role in the chapter and book

**Chapter role:** Replace naive dispatch with vectorized token packing, capacity-aware expert batches, and equivalence tests.

**Reader enters knowing:** Reader can implement top-2 naively and understands the expected outputs.

**Reader leaves with:** Reader can build vectorized dispatch tensors and prove equivalence against the naive reference layer.

**Chapter-level invariant:** Vectorization may change order and layout, but it must not change mathematical output when no overflow occurs.

**This section's bridge sentence:** With efficient dispatch available, we can focus on the training losses that shape router behavior.
...
1. **Opening paragraph.** State the concrete problem: Prove the vectorized implementation matches the simple implementation in the no-overflow case. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use fixed router scores and identical expert weights for both layers.
3. **Visual explanation.** Insert `\input{figures/ch06/fig-equivalence-tests-against-the-naive-layer}` immediately after the setup.
4. **Shape and notation block.** Introduce symbols before the equation: B = batch size, T = sequence length, D = model width, N = B*T flattened token count, E = number of routed experts, k = number of selected routed experts, C = per-expert capacity or slot count, s = expert slot index.
5. **Mechanics and equation.** Insert `\input{equations/ch06/eq-equivalence-tests-against-the-naive-layer}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch06/lst-equivalence-tests-against-the-naive-layer}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch06/tab-equivalence-tests-against-the-naive-layer}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch06/fig-equivalence-tests-against-the-naive-layer.tex` |
| Include command | `\input{figures/ch06/fig-equivalence-tests-against-the-naive-layer}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch06-equivalence-tests-against-the-naive-layer` |
| Caption intent | Two implementation paths producing matching output tensors. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Two implementation paths producing matching output tensors.

**Visual construction notes:** Use slot grids, packed expert batches, and muted gray for inactive capacity slots. The diagram should clarify indices. Use semantic TikZ styles from `styles/tikzstyles.tex`. Do not use raw hex colors in the diagram body. If this is a generated PDF plot, save the plotted PDF under `figures/generated/ch06/` and keep the wrapper responsible only for `\includegraphics`, caption, and label.

### Table placeholder

| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch06/tab-equivalence-tests-against-the-naive-layer.tex` |
| Include command | `\input{tables/ch06/tab-equivalence-tests-against-the-naive-layer}` |
| Label | `tab:ch06-equivalence-tests-against-the-naive-layer` |
| Caption intent | Test matrix for K, E, D, capacity factor, dtype, and overflow setting. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Equivalence tests against the naive layer"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch06/eq-equivalence-tests-against-the-naive-layer.tex` |
| Include command | `\input{equations/ch06/eq-equivalence-tests-against-the-naive-layer}` |
| Label | `eq:ch06-equivalence-tests-against-the-naive-layer` |
| Equation intent | max_abs_diff = max(|y_naive - y_vec|). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch06/lst-equivalence-tests-against-the-naive-layer.tex` |
| Include command | `\input{listings/ch06/lst-equivalence-tests-against-the-naive-layer}` |
| Label | `lst:ch06-equivalence-tests-against-the-naive-layer` |
| Listing intent | pytest-style allclose test comparing naive and vectorized MoE. |

**Code objects to develop or reference:**
- `compare_naive_vectorized`
- `fixed-seed tests`
...
**Primary code location:** `ch06/01_main-chapter-code/ with vectorized_dispatch.py, capacity utilities, and equivalence tests.`

1. Start from the previous section's tested code, not from a fresh implementation.
2. Add only the minimal new object or function needed for **Equivalence tests against the naive layer**.
3. Write a CPU smoke test using the mini-example: Use fixed router scores and identical expert weights for both layers.
4. Preserve the chapter-level invariant: Vectorization may change order and layout, but it must not change mathematical output when no overflow occurs.
...
**Expected code deliverable:** pytest-style allclose test comparing naive and vectorized MoE.
