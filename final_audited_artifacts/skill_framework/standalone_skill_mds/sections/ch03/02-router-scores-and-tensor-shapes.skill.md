---
name: moe-ch03-s02-router-scores-and-tensor-shapes
description: use when developing section 3.2, router scores and tensor shapes, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 3.2 - Router scores and tensor shapes
...
Develop only Section 3.2 of *MoE Models from Scratch*: **Router scores and tensor shapes**.

- Chapter: 3. Experts, routers, and the first MoE layer
- Target section file: `latex_book_skeleton/chapters/ch03/sections/02-router-scores-and-tensor-shapes.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch03/fig-router-scores-and-tensor-shapes.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch03/tab-router-scores-and-tensor-shapes.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch03/eq-router-scores-and-tensor-shapes.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch03/lst-router-scores-and-tensor-shapes.tex`
- Required diagnostic: Assert that the expert dimension equals n_experts and token dimension equals B*T.
...
```text
Develop Section 3.2, "Router scores and tensor shapes", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 3.2 - Router scores and tensor shapes

Develop only Chapter 3.2, **Router scores and tensor shapes**, for *MoE Models from Scratch*.
...
# Detailed section development brief: Chapter 3.2 - Router scores and tensor shapes
...
| Chapter | 3. Experts, routers, and the first MoE layer |
| Section | 3.2. Router scores and tensor shapes |
| Section label slug | `ch03-router-scores-and-tensor-shapes` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch03/sections/02-router-scores-and-tensor-shapes.tex` |
| Planned section include | `\input{chapters/ch03/sections/02-router-scores-and-tensor-shapes}` |
| Previous handoff | Chapter 3.1 - Expert MLPs as named FFNs |
| Next handoff | Chapter 3.3 - Top-k selection without dispatch |
...
**Mini-example to anchor the section:** Flatten x from (B,T,D) to (N,D) and compute router_logits with E=4 experts.
...
**This section's bridge sentence:** Scores become sparse when we select only the top experts per token.
...
1. **Opening paragraph.** State the concrete problem: Define router logits, router probabilities, and the flattening convention used throughout the book. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Flatten x from (B,T,D) to (N,D) and compute router_logits with E=4 experts.
3. **Visual explanation.** Insert `\input{figures/ch03/fig-router-scores-and-tensor-shapes}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch03/eq-router-scores-and-tensor-shapes}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch03/lst-router-scores-and-tensor-shapes}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch03/tab-router-scores-and-tensor-shapes}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch03/fig-router-scores-and-tensor-shapes.tex` |
| Include command | `\input{figures/ch03/fig-router-scores-and-tensor-shapes}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch03-router-scores-and-tensor-shapes` |
| Caption intent | Flattened token matrix multiplied by a router projection to produce an N by E score table. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Flattened token matrix multiplied by a router projection to produce an N by E score table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch03/tab-router-scores-and-tensor-shapes.tex` |
| Include command | `\input{tables/ch03/tab-router-scores-and-tensor-shapes}` |
| Label | `tab:ch03-router-scores-and-tensor-shapes` |
| Caption intent | Router tensor shape contract for x, x_flat, logits, scores, topk_idx, and gates. |
| Required columns | Tensor name | Shape | Created by | Used by | Failure check |

**Planned table rows:**
- row 1: define the primary object for "Router scores and tensor shapes"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch03/eq-router-scores-and-tensor-shapes.tex` |
| Include command | `\input{equations/ch03/eq-router-scores-and-tensor-shapes}` |
| Label | `eq:ch03-router-scores-and-tensor-shapes` |
| Equation intent | router_logits = x_flat W_r + b_r. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch03/lst-router-scores-and-tensor-shapes.tex` |
| Include command | `\input{listings/ch03/lst-router-scores-and-tensor-shapes}` |
| Label | `lst:ch03-router-scores-and-tensor-shapes` |
| Listing intent | Router module that returns logits and score tensors. |

**Code objects to develop or reference:**
- `Router`
- `score/logit shape test`
...
2. Add only the minimal new object or function needed for **Router scores and tensor shapes**.
3. Write a CPU smoke test using the mini-example: Flatten x from (B,T,D) to (N,D) and compute router_logits with E=4 experts.
...
**Expected code deliverable:** Router module that returns logits and score tensors.
