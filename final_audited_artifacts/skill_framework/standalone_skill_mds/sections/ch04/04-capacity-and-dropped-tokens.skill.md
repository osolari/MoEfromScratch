---
name: moe-ch04-s04-capacity-and-dropped-tokens
description: use when developing section 4.4, capacity and dropped tokens, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 4.4 - Capacity and dropped tokens
...
Develop only Section 4.4 of *MoE Models from Scratch*: **Capacity and dropped tokens**.

- Chapter: 4. Top-1 routing and Switch-style dispatch
- Target section file: `latex_book_skeleton/chapters/ch04/sections/04-capacity-and-dropped-tokens.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch04/fig-capacity-and-dropped-tokens.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch04/tab-capacity-and-dropped-tokens.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch04/eq-capacity-and-dropped-tokens.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch04/lst-capacity-and-dropped-tokens.tex`
- Required diagnostic: Report dropped_token_count and per-expert accepted counts.
...
```text
Develop Section 4.4, "Capacity and dropped tokens", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 4.4 - Capacity and dropped tokens
...
| Chapter | 4. Top-1 routing and Switch-style dispatch |
| Section | 4.4. Capacity and dropped tokens |
| Section label slug | `ch04-capacity-and-dropped-tokens` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch04/sections/04-capacity-and-dropped-tokens.tex` |
| Planned section include | `\input{chapters/ch04/sections/04-capacity-and-dropped-tokens}` |
| Previous handoff | Chapter 4.3 - Dispatching tokens to selected experts |
| Next handoff | Chapter 4.5 - Load histograms and failure modes |
...
**Mini-example to anchor the section:** Set capacity=2 for four experts and show what happens when four tokens select expert 0.
...
**This section's bridge sentence:** Capacity exposes routing imbalance; histograms make the imbalance easy to see.
...
1. **Opening paragraph.** State the concrete problem: Introduce the capacity factor and the consequences of too many tokens choosing the same expert. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Set capacity=2 for four experts and show what happens when four tokens select expert 0.
3. **Visual explanation.** Insert `\input{figures/ch04/fig-capacity-and-dropped-tokens}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch04/eq-capacity-and-dropped-tokens}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch04/lst-capacity-and-dropped-tokens}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch04/tab-capacity-and-dropped-tokens}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch04/fig-capacity-and-dropped-tokens.tex` |
| Include command | `\input{figures/ch04/fig-capacity-and-dropped-tokens}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch04-capacity-and-dropped-tokens` |
| Caption intent | Overloaded expert bucket with accepted and dropped token slots. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Overloaded expert bucket with accepted and dropped token slots.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch04/tab-capacity-and-dropped-tokens.tex` |
| Include command | `\input{tables/ch04/tab-capacity-and-dropped-tokens}` |
| Label | `tab:ch04-capacity-and-dropped-tokens` |
| Caption intent | Capacity factor, expert capacity, accepted tokens, dropped tokens, and residual fallback. |
| Required columns | Capacity term | Symbol/code name | Meaning | Edge case | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Capacity and dropped tokens"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch04/eq-capacity-and-dropped-tokens.tex` |
| Include command | `\input{equations/ch04/eq-capacity-and-dropped-tokens}` |
| Label | `eq:ch04-capacity-and-dropped-tokens` |
| Equation intent | capacity = ceil(capacity_factor * N / E). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch04/lst-capacity-and-dropped-tokens.tex` |
| Include command | `\input{listings/ch04/lst-capacity-and-dropped-tokens}` |
| Label | `lst:ch04-capacity-and-dropped-tokens` |
| Listing intent | Capacity-aware top-1 dispatch with dropped-token accounting. |
...
2. Add only the minimal new object or function needed for **Capacity and dropped tokens**.
3. Write a CPU smoke test using the mini-example: Set capacity=2 for four experts and show what happens when four tokens select expert 0.
...
**Expected code deliverable:** Capacity-aware top-1 dispatch with dropped-token accounting.
