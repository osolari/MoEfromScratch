---
name: moe-ch04-s01-why-top-1-routing-is-the-simplest-sparse-c
description: use when developing section 4.1, why top-1 routing is the simplest sparse case, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 4.1 - Why top-1 routing is the simplest sparse case
...
Develop only Section 4.1 of *MoE Models from Scratch*: **Why top-1 routing is the simplest sparse case**.

- Chapter: 4. Top-1 routing and Switch-style dispatch
- Target section file: `latex_book_skeleton/chapters/ch04/sections/01-why-top-1-routing-is-the-simplest-sparse-case.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch04/fig-why-top-1-routing-is-the-simplest-sparse-case.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch04/tab-why-top-1-routing-is-the-simplest-sparse-case.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch04/eq-why-top-1-routing-is-the-simplest-sparse-case.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch04/lst-why-top-1-routing-is-the-simplest-sparse-case.tex`
- Required diagnostic: Verify each token has one expert and one scalar gate.
...
```text
Develop Section 4.1, "Why top-1 routing is the simplest sparse case", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 4.1 - Why top-1 routing is the simplest sparse case

Develop only Chapter 4.1, **Why top-1 routing is the simplest sparse case**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch04/01-why-top-1-routing-is-the-simplest-sparse-case.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch04/sections/01-why-top-1-routing-is-the-simplest-sparse-case.tex`
...
# Detailed section development brief: Chapter 4.1 - Why top-1 routing is the simplest sparse case
...
| Chapter | 4. Top-1 routing and Switch-style dispatch |
| Section | 4.1. Why top-1 routing is the simplest sparse case |
| Section label slug | `ch04-why-top-1-routing-is-the-simplest-sparse-case` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch04/sections/01-why-top-1-routing-is-the-simplest-sparse-case.tex` |
| Planned section include | `\input{chapters/ch04/sections/01-why-top-1-routing-is-the-simplest-sparse-case}` |
| Previous handoff | Chapter 3.6 - Chapter summary and handoff |
| Next handoff | Chapter 4.2 - Assignment masks and expert buckets |
...
**Mini-example to anchor the section:** Route six tokens to four experts with K=1 and mark each token assignment.
...
**This section's bridge sentence:** The single expert ID can be converted into an assignment mask.
...
1. **Opening paragraph.** State the concrete problem: Use one-expert-per-token routing to reduce the dispatch problem to assignment plus combine. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Route six tokens to four experts with K=1 and mark each token assignment.
3. **Visual explanation.** Insert `\input{figures/ch04/fig-why-top-1-routing-is-the-simplest-sparse-case}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch04/eq-why-top-1-routing-is-the-simplest-sparse-case}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch04/lst-why-top-1-routing-is-the-simplest-sparse-case}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch04/tab-why-top-1-routing-is-the-simplest-sparse-case}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch04/fig-why-top-1-routing-is-the-simplest-sparse-case.tex` |
| Include command | `\input{figures/ch04/fig-why-top-1-routing-is-the-simplest-sparse-case}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch04-why-top-1-routing-is-the-simplest-sparse-case` |
| Caption intent | Token rows each connected to one selected expert. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Token rows each connected to one selected expert.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch04/tab-why-top-1-routing-is-the-simplest-sparse-case.tex` |
| Include command | `\input{tables/ch04/tab-why-top-1-routing-is-the-simplest-sparse-case}` |
| Label | `tab:ch04-why-top-1-routing-is-the-simplest-sparse-case` |
| Caption intent | Top-1 routing variables compared with top-k routing variables. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Why top-1 routing is the simplest sparse case"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch04/eq-why-top-1-routing-is-the-simplest-sparse-case.tex` |
| Include command | `\input{equations/ch04/eq-why-top-1-routing-is-the-simplest-sparse-case}` |
| Label | `eq:ch04-why-top-1-routing-is-the-simplest-sparse-case` |
| Equation intent | expert_idx_t = argmax_i score_{t,i}. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch04/lst-why-top-1-routing-is-the-simplest-sparse-case.tex` |
| Include command | `\input{listings/ch04/lst-why-top-1-routing-is-the-simplest-sparse-case}` |
| Label | `lst:ch04-why-top-1-routing-is-the-simplest-sparse-case` |
| Listing intent | Top1Router forward pass returning expert_idx and gate. |

**Code objects to develop or reference:**
- `Top1Router`
- `argmax selection test`
...
2. Add only the minimal new object or function needed for **Why top-1 routing is the simplest sparse case**.
3. Write a CPU smoke test using the mini-example: Route six tokens to four experts with K=1 and mark each token assignment.
...
**Expected code deliverable:** Top1Router forward pass returning expert_idx and gate.
