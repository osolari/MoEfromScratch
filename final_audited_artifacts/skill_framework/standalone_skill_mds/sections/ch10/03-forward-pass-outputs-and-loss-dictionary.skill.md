---
name: moe-ch10-s03-forward-pass-outputs-and-loss-dictionary
description: use when developing section 10.3, forward pass outputs and loss dictionary, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 10.3 - Forward pass outputs and loss dictionary
...
Develop only Section 10.3 of *MoE Models from Scratch*: **Forward pass outputs and loss dictionary**.

- Chapter: 10. Assembling MiniDeepSeekMoE end to end
- Target section file: `latex_book_skeleton/chapters/ch10/sections/03-forward-pass-outputs-and-loss-dictionary.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch10/fig-forward-pass-outputs-and-loss-dictionary.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch10/tab-forward-pass-outputs-and-loss-dictionary.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch10/eq-forward-pass-outputs-and-loss-dictionary.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch10/lst-forward-pass-outputs-and-loss-dictionary.tex`
- Required diagnostic: Unit test for output keys under dense, auxiliary, and bias modes.
...
```text
Develop Section 10.3, "Forward pass outputs and loss dictionary", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 10.3 - Forward pass outputs and loss dictionary

Develop only Chapter 10.3, **Forward pass outputs and loss dictionary**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch10/03-forward-pass-outputs-and-loss-dictionary.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch10/sections/03-forward-pass-outputs-and-loss-dictionary.tex`
...
# Detailed section development brief: Chapter 10.3 - Forward pass outputs and loss dictionary
...
| Chapter | 10. Assembling MiniDeepSeekMoE end to end |
| Section | 10.3. Forward pass outputs and loss dictionary |
| Section label slug | `ch10-forward-pass-outputs-and-loss-dictionary` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch10/sections/03-forward-pass-outputs-and-loss-dictionary.tex` |
| Planned section include | `\input{chapters/ch10/sections/03-forward-pass-outputs-and-loss-dictionary}` |
| Previous handoff | Chapter 10.2 - Decoder block with dense prefix and MoE layers |
| Next handoff | Chapter 10.4 - Training and sampling scripts |
...
**Mini-example to anchor the section:** Forward pass returns logits, lm_loss, total_loss, router_metrics, and optional aux_loss.
...
**This section's bridge sentence:** Training and sampling scripts can now be shared across variants.
...
1. **Opening paragraph.** State the concrete problem: Return logits and structured losses/metrics without making training code guess where values live. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Forward pass returns logits, lm_loss, total_loss, router_metrics, and optional aux_loss.
3. **Visual explanation.** Insert `\input{figures/ch10/fig-forward-pass-outputs-and-loss-dictionary}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch10/eq-forward-pass-outputs-and-loss-dictionary}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch10/lst-forward-pass-outputs-and-loss-dictionary}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch10/tab-forward-pass-outputs-and-loss-dictionary}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch10/fig-forward-pass-outputs-and-loss-dictionary.tex` |
| Include command | `\input{figures/ch10/fig-forward-pass-outputs-and-loss-dictionary}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch10-forward-pass-outputs-and-loss-dictionary` |
| Caption intent | Forward output dictionary with model outputs and nested per-layer router metrics. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Forward output dictionary with model outputs and nested per-layer router metrics.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch10/tab-forward-pass-outputs-and-loss-dictionary.tex` |
| Include command | `\input{tables/ch10/tab-forward-pass-outputs-and-loss-dictionary}` |
| Label | `tab:ch10-forward-pass-outputs-and-loss-dictionary` |
| Caption intent | Output dictionary keys, shapes, and when they are present. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Forward pass outputs and loss dictionary"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch10/eq-forward-pass-outputs-and-loss-dictionary.tex` |
| Include command | `\input{equations/ch10/eq-forward-pass-outputs-and-loss-dictionary}` |
| Label | `eq:ch10-forward-pass-outputs-and-loss-dictionary` |
| Equation intent | total_loss depends on selected balancing mode. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch10/lst-forward-pass-outputs-and-loss-dictionary.tex` |
| Include command | `\input{listings/ch10/lst-forward-pass-outputs-and-loss-dictionary}` |
| Label | `lst:ch10-forward-pass-outputs-and-loss-dictionary` |
| Listing intent | MiniDeepSeekMoE.forward implementation with metric aggregation. |

**Code objects to develop or reference:**
- `forward return dictionary`
- `loss dictionary`
...
2. Add only the minimal new object or function needed for **Forward pass outputs and loss dictionary**.
3. Write a CPU smoke test using the mini-example: Forward pass returns logits, lm_loss, total_loss, router_metrics, and optional aux_loss.
...
**Expected code deliverable:** MiniDeepSeekMoE.forward implementation with metric aggregation.
