---
name: moe-ch07-s01-expert-imbalance-as-a-training-problem
description: use when developing section 7.1, expert imbalance as a training problem, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 7.1 - Expert imbalance as a training problem
...
Develop only Section 7.1 of *MoE Models from Scratch*: **Expert imbalance as a training problem**.

- Chapter: 7. Router training and load-balancing losses
- Target section file: `latex_book_skeleton/chapters/ch07/sections/01-expert-imbalance-as-a-training-problem.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch07/fig-expert-imbalance-as-a-training-problem.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch07/tab-expert-imbalance-as-a-training-problem.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch07/eq-expert-imbalance-as-a-training-problem.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch07/lst-expert-imbalance-as-a-training-problem.tex`
- Required diagnostic: Report max_load/mean_load and number of empty experts.
...
```text
Develop Section 7.1, "Expert imbalance as a training problem", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 7.1 - Expert imbalance as a training problem
...
| Chapter | 7. Router training and load-balancing losses |
| Section | 7.1. Expert imbalance as a training problem |
| Section label slug | `ch07-expert-imbalance-as-a-training-problem` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch07/sections/01-expert-imbalance-as-a-training-problem.tex` |
| Planned section include | `\input{chapters/ch07/sections/01-expert-imbalance-as-a-training-problem}` |
| Previous handoff | Chapter 6.6 - Chapter summary and handoff |
| Next handoff | Chapter 7.2 - Auxiliary load-balancing loss |
...
**Mini-example to anchor the section:** Construct a score table where one expert receives most top-k selections.
...
**This section's bridge sentence:** An auxiliary loss can reward agreement between probability mass and actual expert load.
...
1. **Opening paragraph.** State the concrete problem: Show that a technically correct router can still collapse onto a small number of experts. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Construct a score table where one expert receives most top-k selections.
3. **Visual explanation.** Insert `\input{figures/ch07/fig-expert-imbalance-as-a-training-problem}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch07/eq-expert-imbalance-as-a-training-problem}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch07/lst-expert-imbalance-as-a-training-problem}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch07/tab-expert-imbalance-as-a-training-problem}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch07/fig-expert-imbalance-as-a-training-problem.tex` |
| Include command | `\input{figures/ch07/fig-expert-imbalance-as-a-training-problem}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch07-expert-imbalance-as-a-training-problem` |
| Caption intent | Collapsed routing histogram compared with balanced routing histogram. |
...
**Figure content plan:** Collapsed routing histogram compared with balanced routing histogram.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch07/tab-expert-imbalance-as-a-training-problem.tex` |
| Include command | `\input{tables/ch07/tab-expert-imbalance-as-a-training-problem}` |
| Label | `tab:ch07-expert-imbalance-as-a-training-problem` |
| Caption intent | Symptoms of imbalance and the training behavior they affect. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Expert imbalance as a training problem"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch07/eq-expert-imbalance-as-a-training-problem.tex` |
| Include command | `\input{equations/ch07/eq-expert-imbalance-as-a-training-problem}` |
| Label | `eq:ch07-expert-imbalance-as-a-training-problem` |
| Equation intent | Target average load per expert is N*K/E routes. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch07/lst-expert-imbalance-as-a-training-problem.tex` |
| Include command | `\input{listings/ch07/lst-expert-imbalance-as-a-training-problem}` |
| Label | `lst:ch07-expert-imbalance-as-a-training-problem` |
| Listing intent | Synthetic imbalance generator for router-score diagnostics. |
...
2. Add only the minimal new object or function needed for **Expert imbalance as a training problem**.
3. Write a CPU smoke test using the mini-example: Construct a score table where one expert receives most top-k selections.
...
**Expected code deliverable:** Synthetic imbalance generator for router-score diagnostics.
