---
name: moe-ch08-s01-the-role-of-shared-experts
description: use when developing section 8.1, the role of shared experts, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 8.1 - The role of shared experts
...
Develop only Section 8.1 of *MoE Models from Scratch*: **The role of shared experts**.

- Chapter: 8. Shared experts and fine-grained routed experts
- Target section file: `latex_book_skeleton/chapters/ch08/sections/01-the-role-of-shared-experts.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch08/fig-the-role-of-shared-experts.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch08/tab-the-role-of-shared-experts.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch08/eq-the-role-of-shared-experts.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch08/lst-the-role-of-shared-experts.tex`
- Required diagnostic: Verify shared experts receive gradients for every token, not only selected tokens.
...
```text
Develop Section 8.1, "The role of shared experts", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 8.1 - The role of shared experts

Develop only Chapter 8.1, **The role of shared experts**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch08/01-the-role-of-shared-experts.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch08/sections/01-the-role-of-shared-experts.tex`
...
# Detailed section development brief: Chapter 8.1 - The role of shared experts
...
| Chapter | 8. Shared experts and fine-grained routed experts |
| Section | 8.1. The role of shared experts |
| Section label slug | `ch08-the-role-of-shared-experts` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch08/sections/01-the-role-of-shared-experts.tex` |
| Planned section include | `\input{chapters/ch08/sections/01-the-role-of-shared-experts}` |
| Previous handoff | Chapter 7.6 - Chapter summary and handoff |
| Next handoff | Chapter 8.2 - Fine-grained routed expert segmentation |
...
**Mini-example to anchor the section:** Every token goes through one shared expert while also routing to two routed experts.
...
**This section's bridge sentence:** Routed experts can also be made finer-grained to increase specialization options.
...
1. **Opening paragraph.** State the concrete problem: Explain the always-on expert path as a way to separate common processing from routed specialization. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Every token goes through one shared expert while also routing to two routed experts.
3. **Visual explanation.** Insert `\input{figures/ch08/fig-the-role-of-shared-experts}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch08/eq-the-role-of-shared-experts}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch08/lst-the-role-of-shared-experts}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch08/tab-the-role-of-shared-experts}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch08/fig-the-role-of-shared-experts.tex` |
| Include command | `\input{figures/ch08/fig-the-role-of-shared-experts}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch08-the-role-of-shared-experts` |
| Caption intent | Parallel shared path and routed path merging at the MoE output. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Parallel shared path and routed path merging at the MoE output.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch08/tab-the-role-of-shared-experts.tex` |
| Include command | `\input{tables/ch08/tab-the-role-of-shared-experts}` |
| Label | `tab:ch08-the-role-of-shared-experts` |
| Caption intent | Shared expert path versus routed expert path responsibilities. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "The role of shared experts"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch08/eq-the-role-of-shared-experts.tex` |
| Include command | `\input{equations/ch08/eq-the-role-of-shared-experts}` |
| Label | `eq:ch08-the-role-of-shared-experts` |
| Equation intent | shared_out = sum_s shared_expert_s(x). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch08/lst-the-role-of-shared-experts.tex` |
| Include command | `\input{listings/ch08/lst-the-role-of-shared-experts}` |
| Label | `lst:ch08-the-role-of-shared-experts` |
| Listing intent | SharedExpertMLP module and shared_out calculation. |
...
2. Add only the minimal new object or function needed for **The role of shared experts**.
3. Write a CPU smoke test using the mini-example: Every token goes through one shared expert while also routing to two routed experts.
...
**Expected code deliverable:** SharedExpertMLP module and shared_out calculation.
