---
name: moe-ch01-s03-a-tiny-routing-story
description: use when developing section 1.3, a tiny routing story, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 1.3 - A tiny routing story
...
Develop only Section 1.3 of *MoE Models from Scratch*: **A tiny routing story**.

- Chapter: 1. From dense compute to conditional compute
- Target section file: `latex_book_skeleton/chapters/ch01/sections/03-a-tiny-routing-story.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch01/fig-a-tiny-routing-story.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch01/tab-a-tiny-routing-story.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch01/eq-a-tiny-routing-story.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch01/lst-a-tiny-routing-story.tex`
- Required diagnostic: Check that gates for each token sum to one after top-k normalization.
...
```text
Develop Section 1.3, "A tiny routing story", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 1.3 - A tiny routing story
...
| Chapter | 1. From dense compute to conditional compute |
| Section | 1.3. A tiny routing story |
| Section label slug | `ch01-a-tiny-routing-story` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch01/sections/03-a-tiny-routing-story.tex` |
| Planned section include | `\input{chapters/ch01/sections/03-a-tiny-routing-story}` |
| Previous handoff | Chapter 1.2 - Conditional compute as the central idea |
| Next handoff | Chapter 1.4 - What from scratch means for this book |
...
**Mini-example to anchor the section:** Use tokens [cat, sat, code, runs], four experts, and top-2 routing weights.
...
**This section's bridge sentence:** This toy flow becomes the shape contract for every later implementation.
...
1. **Opening paragraph.** State the concrete problem: Give readers a concrete token-by-token routing example before any model code appears. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use tokens [cat, sat, code, runs], four experts, and top-2 routing weights.
3. **Visual explanation.** Insert `\input{figures/ch01/fig-a-tiny-routing-story}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch01/eq-a-tiny-routing-story}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch01/lst-a-tiny-routing-story}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch01/tab-a-tiny-routing-story}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch01/fig-a-tiny-routing-story.tex` |
| Include command | `\input{figures/ch01/fig-a-tiny-routing-story}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch01-a-tiny-routing-story` |
| Caption intent | A storyboard of router scores, selected experts, expert outputs, and weighted combine. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** A storyboard of router scores, selected experts, expert outputs, and weighted combine.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch01/tab-a-tiny-routing-story.tex` |
| Include command | `\input{tables/ch01/tab-a-tiny-routing-story}` |
| Label | `tab:ch01-a-tiny-routing-story` |
| Caption intent | Toy router-score matrix with top-2 expert choices and normalized gates. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "A tiny routing story"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch01/eq-a-tiny-routing-story.tex` |
| Include command | `\input{equations/ch01/eq-a-tiny-routing-story}` |
| Label | `eq:ch01-a-tiny-routing-story` |
| Equation intent | Weighted expert output y_t = sum gate_i expert_i(x_t). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch01/lst-a-tiny-routing-story.tex` |
| Include command | `\input{listings/ch01/lst-a-tiny-routing-story}` |
| Label | `lst:ch01-a-tiny-routing-story` |
| Listing intent | Manual PyTorch tensor example that applies torch.topk and normalized weights. |
...
2. Add only the minimal new object or function needed for **A tiny routing story**.
3. Write a CPU smoke test using the mini-example: Use tokens [cat, sat, code, runs], four experts, and top-2 routing weights.
...
**Expected code deliverable:** Manual PyTorch tensor example that applies torch.topk and normalized weights.
