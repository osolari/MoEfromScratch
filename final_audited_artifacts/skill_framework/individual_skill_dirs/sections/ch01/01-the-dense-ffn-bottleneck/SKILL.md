---
name: moe-ch01-s01-the-dense-ffn-bottleneck
description: use when developing section 1.1, the dense ffn bottleneck, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 1.1 - The dense FFN bottleneck
...
Develop only Section 1.1 of *MoE Models from Scratch*: **The dense FFN bottleneck**.

- Chapter: 1. From dense compute to conditional compute
- Target section file: `latex_book_skeleton/chapters/ch01/sections/01-the-dense-ffn-bottleneck.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch01/fig-the-dense-ffn-bottleneck.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch01/tab-the-dense-ffn-bottleneck.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch01/eq-the-dense-ffn-bottleneck.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch01/lst-the-dense-ffn-bottleneck.tex`
- Required diagnostic: A small cost table proving that increasing H increases every token path.
...
\input{tables/ch01/tab-the-dense-ffn-bottleneck}
\input{equations/ch01/eq-the-dense-ffn-bottleneck}
\input{listings/ch01/lst-the-dense-ffn-bottleneck}
```
...
```text
Develop Section 1.1, "The dense FFN bottleneck", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 1.1 - The dense FFN bottleneck

Develop only Chapter 1.1, **The dense FFN bottleneck**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch01/01-the-dense-ffn-bottleneck.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch01/sections/01-the-dense-ffn-bottleneck.tex`
...
# Detailed section development brief: Chapter 1.1 - The dense FFN bottleneck
...
| Chapter | 1. From dense compute to conditional compute |
| Section | 1.1. The dense FFN bottleneck |
| Section label slug | `ch01-the-dense-ffn-bottleneck` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch01/sections/01-the-dense-ffn-bottleneck.tex` |
| Planned section include | `\input{chapters/ch01/sections/01-the-dense-ffn-bottleneck}` |
| Previous handoff | Book opening |
| Next handoff | Chapter 1.2 - Conditional compute as the central idea |
...
**Mini-example to anchor the section:** Use a four-token batch with d_model=8 and an FFN expansion factor of 4; count multiply-adds for all tokens.
...
**This section's bridge sentence:** Once the bottleneck is visible, the next question is whether every token really needs the same FFN.
...
1. **Opening paragraph.** State the concrete problem: Show that the dense feed-forward block is the natural place to introduce sparsity because every token pays for every hidden unit. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use a four-token batch with d_model=8 and an FFN expansion factor of 4; count multiply-adds for all tokens.
3. **Visual explanation.** Insert `\input{figures/ch01/fig-the-dense-ffn-bottleneck}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch01/eq-the-dense-ffn-bottleneck}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch01/lst-the-dense-ffn-bottleneck}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch01/tab-the-dense-ffn-bottleneck}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch01/fig-the-dense-ffn-bottleneck.tex` |
| Include command | `\input{figures/ch01/fig-the-dense-ffn-bottleneck}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch01-the-dense-ffn-bottleneck` |
| Caption intent | Dense decoder block with the FFN path highlighted as the always-on compute region. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Dense decoder block with the FFN path highlighted as the always-on compute region.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch01/tab-the-dense-ffn-bottleneck.tex` |
| Include command | `\input{tables/ch01/tab-the-dense-ffn-bottleneck}` |
| Label | `tab:ch01-the-dense-ffn-bottleneck` |
| Caption intent | Parameter and activation shape comparison for attention, dense FFN, and the residual path. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "The dense FFN bottleneck"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch01/eq-the-dense-ffn-bottleneck.tex` |
| Include command | `\input{equations/ch01/eq-the-dense-ffn-bottleneck}` |
| Label | `eq:ch01-the-dense-ffn-bottleneck` |
| Equation intent | Dense FFN cost estimate using D, H, and number of tokens N. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch01/lst-the-dense-ffn-bottleneck.tex` |
| Include command | `\input{listings/ch01/lst-the-dense-ffn-bottleneck}` |
| Label | `lst:ch01-the-dense-ffn-bottleneck` |
| Listing intent | Tiny function that estimates dense FFN parameters and per-token matrix multiplies. |
...
2. Add only the minimal new object or function needed for **The dense FFN bottleneck**.
3. Write a CPU smoke test using the mini-example: Use a four-token batch with d_model=8 and an FFN expansion factor of 4; count multiply-adds for all tokens.
...
**Expected code deliverable:** Tiny function that estimates dense FFN parameters and per-token matrix multiplies.
