---
name: moe-ch12-s03-compute-memory-and-active-parameter-accoun
description: use when developing section 12.3, compute, memory, and active-parameter accounting, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 12.3 - Compute, memory, and active-parameter accounting
...
Develop only Section 12.3 of *MoE Models from Scratch*: **Compute, memory, and active-parameter accounting**.

- Chapter: 12. Inference, performance, and scaling beyond the toy model
- Target section file: `latex_book_skeleton/chapters/ch12/sections/03-compute-memory-and-active-parameter-accounting.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch12/fig-compute-memory-and-active-parameter-accounting.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch12/tab-compute-memory-and-active-parameter-accounting.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch12/eq-compute-memory-and-active-parameter-accounting.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch12/lst-compute-memory-and-active-parameter-accounting.tex`
- Required diagnostic: Check estimates against model.count_parameters breakdown.
...
```text
Develop Section 12.3, "Compute, memory, and active-parameter accounting", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 12.3 - Compute, memory, and active-parameter accounting
...
| Chapter | 12. Inference, performance, and scaling beyond the toy model |
| Section | 12.3. Compute, memory, and active-parameter accounting |
| Section label slug | `ch12-compute-memory-and-active-parameter-accounting` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch12/sections/03-compute-memory-and-active-parameter-accounting.tex` |
| Planned section include | `\input{chapters/ch12/sections/03-compute-memory-and-active-parameter-accounting}` |
| Previous handoff | Chapter 12.2 - Batching routed tokens at inference |
| Next handoff | Chapter 12.4 - Expert parallelism and all-to-all communication |
...
**Mini-example to anchor the section:** Compare dense FFN, 16x2 MoE, and 64x6 teaching config parameter counts.
...
**This section's bridge sentence:** At production scale, experts are often spread across devices, introducing communication.
...
1. **Opening paragraph.** State the concrete problem: Give readers a sober way to discuss total parameters, active parameters, and practical memory costs. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Compare dense FFN, 16x2 MoE, and 64x6 teaching config parameter counts.
3. **Visual explanation.** Insert `\input{figures/ch12/fig-compute-memory-and-active-parameter-accounting}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch12/eq-compute-memory-and-active-parameter-accounting}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch12/lst-compute-memory-and-active-parameter-accounting}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch12/tab-compute-memory-and-active-parameter-accounting}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch12/fig-compute-memory-and-active-parameter-accounting.tex` |
| Include command | `\input{figures/ch12/fig-compute-memory-and-active-parameter-accounting}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch12-compute-memory-and-active-parameter-accounting` |
| Caption intent | Stacked bars for total parameters and active parameters per token. |
...
**Figure content plan:** Stacked bars for total parameters and active parameters per token.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch12/tab-compute-memory-and-active-parameter-accounting.tex` |
| Include command | `\input{tables/ch12/tab-compute-memory-and-active-parameter-accounting}` |
| Label | `tab:ch12-compute-memory-and-active-parameter-accounting` |
| Caption intent | Total parameters, active parameters, estimated activation memory, and expert calls by config. |
| Required columns | Quantity | Dense baseline | MoE path | Caveat | Measurement |

**Planned table rows:**
- row 1: define the primary object for "Compute, memory, and active-parameter accounting"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch12/eq-compute-memory-and-active-parameter-accounting.tex` |
| Include command | `\input{equations/ch12/eq-compute-memory-and-active-parameter-accounting}` |
| Label | `eq:ch12-compute-memory-and-active-parameter-accounting` |
| Equation intent | Active expert FFN parameters per token approximately K times one routed expert plus shared experts. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch12/lst-compute-memory-and-active-parameter-accounting.tex` |
| Include command | `\input{listings/ch12/lst-compute-memory-and-active-parameter-accounting}` |
| Label | `lst:ch12-compute-memory-and-active-parameter-accounting` |
| Listing intent | estimate_moe_compute.py producing parameter and active-compute estimates. |

**Code objects to develop or reference:**
- `active_parameter_counter`
...
2. Add only the minimal new object or function needed for **Compute, memory, and active-parameter accounting**.
3. Write a CPU smoke test using the mini-example: Compare dense FFN, 16x2 MoE, and 64x6 teaching config parameter counts.
...
**Expected code deliverable:** estimate_moe_compute.py producing parameter and active-compute estimates.
