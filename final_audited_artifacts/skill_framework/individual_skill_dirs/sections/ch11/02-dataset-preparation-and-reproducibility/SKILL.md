---
name: moe-ch11-s02-dataset-preparation-and-reproducibility
description: use when developing section 11.2, dataset preparation and reproducibility, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 11.2 - Dataset preparation and reproducibility
...
Develop only Section 11.2 of *MoE Models from Scratch*: **Dataset preparation and reproducibility**.

- Chapter: 11. Training, diagnostics, and controlled experiments
- Target section file: `latex_book_skeleton/chapters/ch11/sections/02-dataset-preparation-and-reproducibility.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch11/fig-dataset-preparation-and-reproducibility.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch11/tab-dataset-preparation-and-reproducibility.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch11/eq-dataset-preparation-and-reproducibility.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch11/lst-dataset-preparation-and-reproducibility.tex`
- Required diagnostic: Print token counts, split sizes, and a decoded sample from each split.
...
```text
Develop Section 11.2, "Dataset preparation and reproducibility", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 11.2 - Dataset preparation and reproducibility

Develop only Chapter 11.2, **Dataset preparation and reproducibility**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch11/02-dataset-preparation-and-reproducibility.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch11/sections/02-dataset-preparation-and-reproducibility.tex`
...
# Detailed section development brief: Chapter 11.2 - Dataset preparation and reproducibility
...
| Chapter | 11. Training, diagnostics, and controlled experiments |
| Section | 11.2. Dataset preparation and reproducibility |
| Section label slug | `ch11-dataset-preparation-and-reproducibility` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch11/sections/02-dataset-preparation-and-reproducibility.tex` |
| Planned section include | `\input{chapters/ch11/sections/02-dataset-preparation-and-reproducibility}` |
| Previous handoff | Chapter 11.1 - Experiment design for a from-scratch book |
| Next handoff | Chapter 11.3 - Loss curves and routing dashboards |
...
**Mini-example to anchor the section:** Prepare a small corpus split into train and validation binary or tensor files.
...
**This section's bridge sentence:** Training logs must include both language and routing metrics.
...
1. **Opening paragraph.** State the concrete problem: Make dataset setup deterministic and lightweight enough for readers to rerun. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Prepare a small corpus split into train and validation binary or tensor files.
3. **Visual explanation.** Insert `\input{figures/ch11/fig-dataset-preparation-and-reproducibility}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch11/eq-dataset-preparation-and-reproducibility}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch11/lst-dataset-preparation-and-reproducibility}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch11/tab-dataset-preparation-and-reproducibility}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch11/fig-dataset-preparation-and-reproducibility.tex` |
| Include command | `\input{figures/ch11/fig-dataset-preparation-and-reproducibility}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch11-dataset-preparation-and-reproducibility` |
| Caption intent | Raw text to tokenized train/validation artifacts. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Raw text to tokenized train/validation artifacts.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch11/tab-dataset-preparation-and-reproducibility.tex` |
| Include command | `\input{tables/ch11/tab-dataset-preparation-and-reproducibility}` |
| Label | `tab:ch11-dataset-preparation-and-reproducibility` |
| Caption intent | Dataset artifacts, paths, and regeneration commands. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Dataset preparation and reproducibility"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch11/eq-dataset-preparation-and-reproducibility.tex` |
| Include command | `\input{equations/ch11/eq-dataset-preparation-and-reproducibility}` |
| Label | `eq:ch11-dataset-preparation-and-reproducibility` |
| Equation intent | Train/validation split ratio and token count definitions. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch11/lst-dataset-preparation-and-reproducibility.tex` |
| Include command | `\input{listings/ch11/lst-dataset-preparation-and-reproducibility}` |
| Label | `lst:ch11-dataset-preparation-and-reproducibility` |
| Listing intent | prepare.py script with deterministic split and saved metadata. |

**Code objects to develop or reference:**
- `prepare_data.py`
- `seed/fingerprint logging`
...
2. Add only the minimal new object or function needed for **Dataset preparation and reproducibility**.
3. Write a CPU smoke test using the mini-example: Prepare a small corpus split into train and validation binary or tensor files.
...
**Expected code deliverable:** prepare.py script with deterministic split and saved metadata.
