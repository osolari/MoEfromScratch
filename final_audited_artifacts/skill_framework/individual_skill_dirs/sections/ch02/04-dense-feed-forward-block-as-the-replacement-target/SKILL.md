---
name: moe-ch02-s04-dense-feed-forward-block-as-the-replacemen
description: use when developing section 2.4, dense feed-forward block as the replacement target, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 2.4 - Dense feed-forward block as the replacement target
...
Develop only Section 2.4 of *MoE Models from Scratch*: **Dense feed-forward block as the replacement target**.

- Chapter: 2. A dense Transformer baseline from scratch
- Target section file: `latex_book_skeleton/chapters/ch02/sections/04-dense-feed-forward-block-as-the-replacement-target.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch02/fig-dense-feed-forward-block-as-the-replacement-target.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch02/tab-dense-feed-forward-block-as-the-replacement-target.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch02/eq-dense-feed-forward-block-as-the-replacement-target.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch02/lst-dense-feed-forward-block-as-the-replacement-target.tex`
- Required diagnostic: Confirm FFN preserves the final D dimension.
...
```text
Develop Section 2.4, "Dense feed-forward block as the replacement target", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 2.4 - Dense feed-forward block as the replacement target

Develop only Chapter 2.4, **Dense feed-forward block as the replacement target**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch02/04-dense-feed-forward-block-as-the-replacement-target.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch02/sections/04-dense-feed-forward-block-as-the-replacement-target.tex`
...
# Detailed section development brief: Chapter 2.4 - Dense feed-forward block as the replacement target
...
| Chapter | 2. A dense Transformer baseline from scratch |
| Section | 2.4. Dense feed-forward block as the replacement target |
| Section label slug | `ch02-dense-feed-forward-block-as-the-replacement-target` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch02/sections/04-dense-feed-forward-block-as-the-replacement-target.tex` |
| Planned section include | `\input{chapters/ch02/sections/04-dense-feed-forward-block-as-the-replacement-target}` |
| Previous handoff | Chapter 2.3 - Causal self-attention as the context path |
| Next handoff | Chapter 2.5 - Training loop and sampling baseline |
...
**Mini-example to anchor the section:** Apply the same MLP to every token in a B*T flattened view.
...
**This section's bridge sentence:** Attention and FFN combine inside a residual decoder block.
...
1. **Opening paragraph.** State the concrete problem: Implement the dense FFN in a way that makes the later expert replacement obvious. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Apply the same MLP to every token in a B*T flattened view.
3. **Visual explanation.** Insert `\input{figures/ch02/fig-dense-feed-forward-block-as-the-replacement-target}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch02/eq-dense-feed-forward-block-as-the-replacement-target}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch02/lst-dense-feed-forward-block-as-the-replacement-target}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch02/tab-dense-feed-forward-block-as-the-replacement-target}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch02/fig-dense-feed-forward-block-as-the-replacement-target.tex` |
| Include command | `\input{figures/ch02/fig-dense-feed-forward-block-as-the-replacement-target}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch02-dense-feed-forward-block-as-the-replacement-target` |
| Caption intent | Dense FFN applied independently to each token row. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Dense FFN applied independently to each token row.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch02/tab-dense-feed-forward-block-as-the-replacement-target.tex` |
| Include command | `\input{tables/ch02/tab-dense-feed-forward-block-as-the-replacement-target}` |
| Label | `tab:ch02-dense-feed-forward-block-as-the-replacement-target` |
| Caption intent | Dense FFN parameters compared with one future expert MLP. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Dense feed-forward block as the replacement target"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch02/eq-dense-feed-forward-block-as-the-replacement-target.tex` |
| Include command | `\input{equations/ch02/eq-dense-feed-forward-block-as-the-replacement-target}` |
| Label | `eq:ch02-dense-feed-forward-block-as-the-replacement-target` |
| Equation intent | FFN(x) = W_2 activation(W_1 x + b_1) + b_2. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch02/lst-dense-feed-forward-block-as-the-replacement-target.tex` |
| Include command | `\input{listings/ch02/lst-dense-feed-forward-block-as-the-replacement-target}` |
| Label | `lst:ch02-dense-feed-forward-block-as-the-replacement-target` |
| Listing intent | FeedForward module with Linear, activation, Linear. |

**Code objects to develop or reference:**
- `DenseFFN`
- `TransformerBlock dense path`
...
2. Add only the minimal new object or function needed for **Dense feed-forward block as the replacement target**.
3. Write a CPU smoke test using the mini-example: Apply the same MLP to every token in a B*T flattened view.
...
**Expected code deliverable:** FeedForward module with Linear, activation, Linear.
