---
name: moe-ch03-s01-expert-mlps-as-named-ffns
description: use when developing section 3.1, expert mlps as named ffns, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 3.1 - Expert MLPs as named FFNs
...
Develop only Section 3.1 of *MoE Models from Scratch*: **Expert MLPs as named FFNs**.

- Chapter: 3. Experts, routers, and the first MoE layer
- Target section file: `latex_book_skeleton/chapters/ch03/sections/01-expert-mlps-as-named-ffns.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch03/fig-expert-mlps-as-named-ffns.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch03/tab-expert-mlps-as-named-ffns.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch03/eq-expert-mlps-as-named-ffns.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch03/lst-expert-mlps-as-named-ffns.tex`
- Required diagnostic: Run all experts on the same token and compare output shapes.
...
```text
Develop Section 3.1, "Expert MLPs as named FFNs", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 3.1 - Expert MLPs as named FFNs

Develop only Chapter 3.1, **Expert MLPs as named FFNs**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch03/01-expert-mlps-as-named-ffns.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch03/sections/01-expert-mlps-as-named-ffns.tex`
...
# Detailed section development brief: Chapter 3.1 - Expert MLPs as named FFNs
...
| Chapter | 3. Experts, routers, and the first MoE layer |
| Section | 3.1. Expert MLPs as named FFNs |
| Section label slug | `ch03-expert-mlps-as-named-ffns` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch03/sections/01-expert-mlps-as-named-ffns.tex` |
| Planned section include | `\input{chapters/ch03/sections/01-expert-mlps-as-named-ffns}` |
| Previous handoff | Chapter 2.6 - Baseline diagnostics and comparison slots |
| Next handoff | Chapter 3.2 - Router scores and tensor shapes |
...
**Mini-example to anchor the section:** Clone the dense FFN four times and label the copies expert 0 through expert 3.
...
**This section's bridge sentence:** Experts need a token-dependent selection mechanism: the router.
...
1. **Opening paragraph.** State the concrete problem: Demystify experts by deriving them directly from the dense FFN module. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Clone the dense FFN four times and label the copies expert 0 through expert 3.
3. **Visual explanation.** Insert `\input{figures/ch03/fig-expert-mlps-as-named-ffns}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch03/eq-expert-mlps-as-named-ffns}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch03/lst-expert-mlps-as-named-ffns}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch03/tab-expert-mlps-as-named-ffns}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch03/fig-expert-mlps-as-named-ffns.tex` |
| Include command | `\input{figures/ch03/fig-expert-mlps-as-named-ffns}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch03-expert-mlps-as-named-ffns` |
| Caption intent | One dense FFN replaced by a bank of same-shape expert MLPs. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** One dense FFN replaced by a bank of same-shape expert MLPs.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch03/tab-expert-mlps-as-named-ffns.tex` |
| Include command | `\input{tables/ch03/tab-expert-mlps-as-named-ffns}` |
| Label | `tab:ch03-expert-mlps-as-named-ffns` |
| Caption intent | Dense FFN versus expert bank parameters, active parameters, and outputs. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Expert MLPs as named FFNs"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch03/eq-expert-mlps-as-named-ffns.tex` |
| Include command | `\input{equations/ch03/eq-expert-mlps-as-named-ffns}` |
| Label | `eq:ch03-expert-mlps-as-named-ffns` |
| Equation intent | expert_i(x) maps R^D to R^D for every expert i. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch03/lst-expert-mlps-as-named-ffns.tex` |
| Include command | `\input{listings/ch03/lst-expert-mlps-as-named-ffns}` |
| Label | `lst:ch03-expert-mlps-as-named-ffns` |
| Listing intent | ExpertMLP class with the same input-output contract as the dense FFN. |
...
2. Add only the minimal new object or function needed for **Expert MLPs as named FFNs**.
3. Write a CPU smoke test using the mini-example: Clone the dense FFN four times and label the copies expert 0 through expert 3.
...
**Expected code deliverable:** ExpertMLP class with the same input-output contract as the dense FFN.
