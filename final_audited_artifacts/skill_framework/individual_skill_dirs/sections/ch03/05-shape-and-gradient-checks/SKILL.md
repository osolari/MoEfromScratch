---
name: moe-ch03-s05-shape-and-gradient-checks
description: use when developing section 3.5, shape and gradient checks, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 3.5 - Shape and gradient checks
...
Develop only Section 3.5 of *MoE Models from Scratch*: **Shape and gradient checks**.

- Chapter: 3. Experts, routers, and the first MoE layer
- Target section file: `latex_book_skeleton/chapters/ch03/sections/05-shape-and-gradient-checks.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch03/fig-shape-and-gradient-checks.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch03/tab-shape-and-gradient-checks.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch03/eq-shape-and-gradient-checks.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch03/lst-shape-and-gradient-checks.tex`
- Required diagnostic: Count selected expert IDs and check nonzero gradients for selected expert parameters.
...
```text
Develop Section 3.5, "Shape and gradient checks", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 3.5 - Shape and gradient checks

Develop only Chapter 3.5, **Shape and gradient checks**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch03/05-shape-and-gradient-checks.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch03/sections/05-shape-and-gradient-checks.tex`
...
# Detailed section development brief: Chapter 3.5 - Shape and gradient checks
...
| Chapter | 3. Experts, routers, and the first MoE layer |
| Section | 3.5. Shape and gradient checks |
| Section label slug | `ch03-shape-and-gradient-checks` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch03/sections/05-shape-and-gradient-checks.tex` |
| Planned section include | `\input{chapters/ch03/sections/05-shape-and-gradient-checks}` |
| Previous handoff | Chapter 3.4 - Naive token-by-token dispatch |
| Next handoff | Chapter 3.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Run a two-layer smoke model and confirm all experts receive gradients when selected.
...
**This section's bridge sentence:** The next chapter simplifies to top-1 routing to teach sparse dispatch one expert at a time.
...
1. **Opening paragraph.** State the concrete problem: Make the first MoE layer testable before introducing more routing variants. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Run a two-layer smoke model and confirm all experts receive gradients when selected.
3. **Visual explanation.** Insert `\input{figures/ch03/fig-shape-and-gradient-checks}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch03/eq-shape-and-gradient-checks}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch03/lst-shape-and-gradient-checks}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch03/tab-shape-and-gradient-checks}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch03/fig-shape-and-gradient-checks.tex` |
| Include command | `\input{figures/ch03/fig-shape-and-gradient-checks}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch03-shape-and-gradient-checks` |
| Caption intent | Gradient flow diagram from loss through combine weights into router and selected experts. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Gradient flow diagram from loss through combine weights into router and selected experts.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch03/tab-shape-and-gradient-checks.tex` |
| Include command | `\input{tables/ch03/tab-shape-and-gradient-checks}` |
| Label | `tab:ch03-shape-and-gradient-checks` |
| Caption intent | Test cases for shapes, gate sums, selected expert counts, and gradients. |
| Required columns | Tensor name | Shape | Created by | Used by | Failure check |

**Planned table rows:**
- row 1: define the primary object for "Shape and gradient checks"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch03/eq-shape-and-gradient-checks.tex` |
| Include command | `\input{equations/ch03/eq-shape-and-gradient-checks}` |
| Label | `eq:ch03-shape-and-gradient-checks` |
| Equation intent | Loss gradient reaches gate and expert paths through the weighted sum. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch03/lst-shape-and-gradient-checks.tex` |
| Include command | `\input{listings/ch03/lst-shape-and-gradient-checks}` |
| Label | `lst:ch03-shape-and-gradient-checks` |
| Listing intent | Unit tests for MoELayer output shape and gradient presence. |
...
2. Add only the minimal new object or function needed for **Shape and gradient checks**.
3. Write a CPU smoke test using the mini-example: Run a two-layer smoke model and confirm all experts receive gradients when selected.
...
**Expected code deliverable:** Unit tests for MoELayer output shape and gradient presence.
