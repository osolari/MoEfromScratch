---
name: moe-ch05-s04-comparing-top-1-and-top-2-diagnostics
description: use when developing section 5.4, comparing top-1 and top-2 diagnostics, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 5.4 - Comparing top-1 and top-2 diagnostics
...
Develop only Section 5.4 of *MoE Models from Scratch*: **Comparing top-1 and top-2 diagnostics**.

- Chapter: 5. Top-2 routing and weighted expert combination
- Target section file: `latex_book_skeleton/chapters/ch05/sections/04-comparing-top-1-and-top-2-diagnostics.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch05/fig-comparing-top-1-and-top-2-diagnostics.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch05/tab-comparing-top-1-and-top-2-diagnostics.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch05/eq-comparing-top-1-and-top-2-diagnostics.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch05/lst-comparing-top-1-and-top-2-diagnostics.tex`
- Required diagnostic: Save a comparison table and one histogram per routing mode.
...
```text
Develop Section 5.4, "Comparing top-1 and top-2 diagnostics", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 5.4 - Comparing top-1 and top-2 diagnostics

Develop only Chapter 5.4, **Comparing top-1 and top-2 diagnostics**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch05/04-comparing-top-1-and-top-2-diagnostics.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch05/sections/04-comparing-top-1-and-top-2-diagnostics.tex`
...
# Detailed section development brief: Chapter 5.4 - Comparing top-1 and top-2 diagnostics
...
| Chapter | 5. Top-2 routing and weighted expert combination |
| Section | 5.4. Comparing top-1 and top-2 diagnostics |
| Section label slug | `ch05-comparing-top-1-and-top-2-diagnostics` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch05/sections/04-comparing-top-1-and-top-2-diagnostics.tex` |
| Planned section include | `\input{chapters/ch05/sections/04-comparing-top-1-and-top-2-diagnostics}` |
| Previous handoff | Chapter 5.3 - Dispatch for two expert paths per token |
| Next handoff | Chapter 5.5 - Numerical stability in top-k gates |
...
**Mini-example to anchor the section:** Run the same random batch through top-1 and top-2 routers with the same scores.
...
**This section's bridge sentence:** Once routing variants share diagnostics, the next challenge is making dispatch less loop-heavy.
...
1. **Opening paragraph.** State the concrete problem: Use the same metrics to show how K changes load, drop rate, and routing entropy. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Run the same random batch through top-1 and top-2 routers with the same scores.
3. **Visual explanation.** Insert `\input{figures/ch05/fig-comparing-top-1-and-top-2-diagnostics}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch05/eq-comparing-top-1-and-top-2-diagnostics}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch05/lst-comparing-top-1-and-top-2-diagnostics}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch05/tab-comparing-top-1-and-top-2-diagnostics}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch05/fig-comparing-top-1-and-top-2-diagnostics.tex` |
| Include command | `\input{figures/ch05/fig-comparing-top-1-and-top-2-diagnostics}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch05-comparing-top-1-and-top-2-diagnostics` |
| Caption intent | Side-by-side expert load histograms for K=1 and K=2. |
...
**Figure content plan:** Side-by-side expert load histograms for K=1 and K=2.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch05/tab-comparing-top-1-and-top-2-diagnostics.tex` |
| Include command | `\input{tables/ch05/tab-comparing-top-1-and-top-2-diagnostics}` |
| Label | `tab:ch05-comparing-top-1-and-top-2-diagnostics` |
| Caption intent | Comparison rows for active calls, max load, entropy, and gate concentration. |
| Required columns | Metric or run | What it measures | Expected healthy pattern | Warning pattern | Where logged |

**Planned table rows:**
- row 1: define the primary object for "Comparing top-1 and top-2 diagnostics"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch05/eq-comparing-top-1-and-top-2-diagnostics.tex` |
| Include command | `\input{equations/ch05/eq-comparing-top-1-and-top-2-diagnostics}` |
| Label | `eq:ch05-comparing-top-1-and-top-2-diagnostics` |
| Equation intent | Active expert calls equal N*K before capacity limits. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch05/lst-comparing-top-1-and-top-2-diagnostics.tex` |
| Include command | `\input{listings/ch05/lst-comparing-top-1-and-top-2-diagnostics}` |
| Label | `lst:ch05-comparing-top-1-and-top-2-diagnostics` |
| Listing intent | compare_routing_modes(router_scores, k_values=[1,2]). |

**Code objects to develop or reference:**
- `TopKRouter(k=2)`
- `two-path routing metadata`
...
2. Add only the minimal new object or function needed for **Comparing top-1 and top-2 diagnostics**.
3. Write a CPU smoke test using the mini-example: Run the same random batch through top-1 and top-2 routers with the same scores.
...
**Expected code deliverable:** compare_routing_modes(router_scores, k_values=[1,2]).
