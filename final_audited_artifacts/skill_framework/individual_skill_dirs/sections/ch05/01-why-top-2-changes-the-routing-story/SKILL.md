---
name: moe-ch05-s01-why-top-2-changes-the-routing-story
description: use when developing section 5.1, why top-2 changes the routing story, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 5.1 - Why top-2 changes the routing story
...
Develop only Section 5.1 of *MoE Models from Scratch*: **Why top-2 changes the routing story**.

- Chapter: 5. Top-2 routing and weighted expert combination
- Target section file: `latex_book_skeleton/chapters/ch05/sections/01-why-top-2-changes-the-routing-story.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch05/fig-why-top-2-changes-the-routing-story.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch05/tab-why-top-2-changes-the-routing-story.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch05/eq-why-top-2-changes-the-routing-story.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch05/lst-why-top-2-changes-the-routing-story.tex`
- Required diagnostic: Compare active expert calls per token for K=1 and K=2.
...
```text
Develop Section 5.1, "Why top-2 changes the routing story", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 5.1 - Why top-2 changes the routing story

Develop only Chapter 5.1, **Why top-2 changes the routing story**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch05/01-why-top-2-changes-the-routing-story.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch05/sections/01-why-top-2-changes-the-routing-story.tex`
...
# Detailed section development brief: Chapter 5.1 - Why top-2 changes the routing story
...
| Chapter | 5. Top-2 routing and weighted expert combination |
| Section | 5.1. Why top-2 changes the routing story |
| Section label slug | `ch05-why-top-2-changes-the-routing-story` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch05/sections/01-why-top-2-changes-the-routing-story.tex` |
| Planned section include | `\input{chapters/ch05/sections/01-why-top-2-changes-the-routing-story}` |
| Previous handoff | Chapter 4.6 - Chapter summary and handoff |
| Next handoff | Chapter 5.2 - Normalizing selected router scores |
...
**Mini-example to anchor the section:** Use four tokens and four experts; each token chooses two experts with different weights.
...
**This section's bridge sentence:** To make top-2 work, selected scores must become stable combine weights.
...
1. **Opening paragraph.** State the concrete problem: Explain top-2 routing as sparse ensemble behavior at the token level. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use four tokens and four experts; each token chooses two experts with different weights.
3. **Visual explanation.** Insert `\input{figures/ch05/fig-why-top-2-changes-the-routing-story}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch05/eq-why-top-2-changes-the-routing-story}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch05/lst-why-top-2-changes-the-routing-story}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch05/tab-why-top-2-changes-the-routing-story}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch05/fig-why-top-2-changes-the-routing-story.tex` |
| Include command | `\input{figures/ch05/fig-why-top-2-changes-the-routing-story}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch05-why-top-2-changes-the-routing-story` |
| Caption intent | Token routes split into two weighted paths. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Token routes split into two weighted paths.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch05/tab-why-top-2-changes-the-routing-story.tex` |
| Include command | `\input{tables/ch05/tab-why-top-2-changes-the-routing-story}` |
| Label | `tab:ch05-why-top-2-changes-the-routing-story` |
| Caption intent | Top-1 versus top-2 routing behavior, cost, and diagnostics. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Why top-2 changes the routing story"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch05/eq-why-top-2-changes-the-routing-story.tex` |
| Include command | `\input{equations/ch05/eq-why-top-2-changes-the-routing-story}` |
| Label | `eq:ch05-why-top-2-changes-the-routing-story` |
| Equation intent | Each selected set S_t contains K=2 expert IDs. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch05/lst-why-top-2-changes-the-routing-story.tex` |
| Include command | `\input{listings/ch05/lst-why-top-2-changes-the-routing-story}` |
| Label | `lst:ch05-why-top-2-changes-the-routing-story` |
| Listing intent | Config switch from K=1 to K=2 and resulting tensor shapes. |
...
2. Add only the minimal new object or function needed for **Why top-2 changes the routing story**.
3. Write a CPU smoke test using the mini-example: Use four tokens and four experts; each token chooses two experts with different weights.
...
**Expected code deliverable:** Config switch from K=1 to K=2 and resulting tensor shapes.
