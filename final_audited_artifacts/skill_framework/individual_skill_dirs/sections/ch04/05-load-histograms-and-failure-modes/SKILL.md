---
name: moe-ch04-s05-load-histograms-and-failure-modes
description: use when developing section 4.5, load histograms and failure modes, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 4.5 - Load histograms and failure modes
...
Develop only Section 4.5 of *MoE Models from Scratch*: **Load histograms and failure modes**.

- Chapter: 4. Top-1 routing and Switch-style dispatch
- Target section file: `latex_book_skeleton/chapters/ch04/sections/05-load-histograms-and-failure-modes.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch04/fig-load-histograms-and-failure-modes.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch04/tab-load-histograms-and-failure-modes.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch04/eq-load-histograms-and-failure-modes.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch04/lst-load-histograms-and-failure-modes.tex`
- Required diagnostic: Save a histogram and a warning if max load is much larger than average load.
...
```text
Develop Section 4.5, "Load histograms and failure modes", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 4.5 - Load histograms and failure modes

Develop only Chapter 4.5, **Load histograms and failure modes**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch04/05-load-histograms-and-failure-modes.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch04/sections/05-load-histograms-and-failure-modes.tex`
...
# Detailed section development brief: Chapter 4.5 - Load histograms and failure modes
...
| Chapter | 4. Top-1 routing and Switch-style dispatch |
| Section | 4.5. Load histograms and failure modes |
| Section label slug | `ch04-load-histograms-and-failure-modes` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch04/sections/05-load-histograms-and-failure-modes.tex` |
| Planned section include | `\input{chapters/ch04/sections/05-load-histograms-and-failure-modes}` |
| Previous handoff | Chapter 4.4 - Capacity and dropped tokens |
| Next handoff | Chapter 4.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Create a synthetic router that sends most tokens to one expert and plot the load.
...
**This section's bridge sentence:** Top-1 is simple but brittle; top-2 routing gives each token a backup path and a weighted combine.
...
1. **Opening paragraph.** State the concrete problem: Make expert collapse and underuse visible with simple diagnostics. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Create a synthetic router that sends most tokens to one expert and plot the load.
3. **Visual explanation.** Insert `\input{figures/ch04/fig-load-histograms-and-failure-modes}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch04/eq-load-histograms-and-failure-modes}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch04/lst-load-histograms-and-failure-modes}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch04/tab-load-histograms-and-failure-modes}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch04/fig-load-histograms-and-failure-modes.tex` |
| Include command | `\input{figures/ch04/fig-load-histograms-and-failure-modes}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch04-load-histograms-and-failure-modes` |
| Caption intent | Expert-load histogram with one overloaded expert highlighted in imbalance red. |
...
**Figure content plan:** Expert-load histogram with one overloaded expert highlighted in imbalance red.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch04/tab-load-histograms-and-failure-modes.tex` |
| Include command | `\input{tables/ch04/tab-load-histograms-and-failure-modes}` |
| Label | `tab:ch04-load-histograms-and-failure-modes` |
| Caption intent | Failure modes: expert collapse, empty experts, high drop rate, low entropy. |
| Required columns | Metric or run | What it measures | Expected healthy pattern | Warning pattern | Where logged |

**Planned table rows:**
- row 1: define the primary object for "Load histograms and failure modes"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch04/eq-load-histograms-and-failure-modes.tex` |
| Include command | `\input{equations/ch04/eq-load-histograms-and-failure-modes}` |
| Label | `eq:ch04-load-histograms-and-failure-modes` |
| Equation intent | load_fraction_i = count_i / N. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch04/lst-load-histograms-and-failure-modes.tex` |
| Include command | `\input{listings/ch04/lst-load-histograms-and-failure-modes}` |
| Label | `lst:ch04-load-histograms-and-failure-modes` |
| Listing intent | Function that computes expert_load, load_fraction, and routing_entropy. |

**Code objects to develop or reference:**
- `load histogram utility`
- `failure-mode report`
...
2. Add only the minimal new object or function needed for **Load histograms and failure modes**.
3. Write a CPU smoke test using the mini-example: Create a synthetic router that sends most tokens to one expert and plot the load.
...
**Expected code deliverable:** Function that computes expert_load, load_fraction, and routing_entropy.
