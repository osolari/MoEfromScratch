---
name: moe-ch08-s03-the-minideepseekmoe-layer-contract
description: use when developing section 8.3, the minideepseekmoe layer contract, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 8.3 - The MiniDeepSeekMoE layer contract
...
Develop only Section 8.3 of *MoE Models from Scratch*: **The MiniDeepSeekMoE layer contract**.

- Chapter: 8. Shared experts and fine-grained routed experts
- Target section file: `latex_book_skeleton/chapters/ch08/sections/03-the-minideepseekmoe-layer-contract.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch08/fig-the-minideepseekmoe-layer-contract.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch08/tab-the-minideepseekmoe-layer-contract.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch08/eq-the-minideepseekmoe-layer-contract.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch08/lst-the-minideepseekmoe-layer-contract.tex`
- Required diagnostic: Assert layer output shape equals input shape under smoke and default configs.
...
```text
Develop Section 8.3, "The MiniDeepSeekMoE layer contract", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 8.3 - The MiniDeepSeekMoE layer contract

Develop only Chapter 8.3, **The MiniDeepSeekMoE layer contract**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch08/03-the-minideepseekmoe-layer-contract.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch08/sections/03-the-minideepseekmoe-layer-contract.tex`
...
# Detailed section development brief: Chapter 8.3 - The MiniDeepSeekMoE layer contract
...
| Chapter | 8. Shared experts and fine-grained routed experts |
| Section | 8.3. The MiniDeepSeekMoE layer contract |
| Section label slug | `ch08-the-minideepseekmoe-layer-contract` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch08/sections/03-the-minideepseekmoe-layer-contract.tex` |
| Planned section include | `\input{chapters/ch08/sections/03-the-minideepseekmoe-layer-contract}` |
| Previous handoff | Chapter 8.2 - Fine-grained routed expert segmentation |
| Next handoff | Chapter 8.4 - Combining shared and routed outputs |
...
**Mini-example to anchor the section:** Use D=128, E=4 routed experts, one shared expert, and K=2 in the smoke config.
...
**This section's bridge sentence:** The forward pass computes shared and routed paths side by side.
...
1. **Opening paragraph.** State the concrete problem: Define the final layer interface that later chapters assemble into the full model. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use D=128, E=4 routed experts, one shared expert, and K=2 in the smoke config.
3. **Visual explanation.** Insert `\input{figures/ch08/fig-the-minideepseekmoe-layer-contract}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch08/eq-the-minideepseekmoe-layer-contract}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch08/lst-the-minideepseekmoe-layer-contract}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch08/tab-the-minideepseekmoe-layer-contract}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch08/fig-the-minideepseekmoe-layer-contract.tex` |
| Include command | `\input{figures/ch08/fig-the-minideepseekmoe-layer-contract}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch08-the-minideepseekmoe-layer-contract` |
| Caption intent | MiniDeepSeekMoE layer block with router, routed experts, shared experts, combine, and residual output. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** MiniDeepSeekMoE layer block with router, routed experts, shared experts, combine, and residual output.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch08/tab-the-minideepseekmoe-layer-contract.tex` |
| Include command | `\input{tables/ch08/tab-the-minideepseekmoe-layer-contract}` |
| Label | `tab:ch08-the-minideepseekmoe-layer-contract` |
| Caption intent | Final layer tensor contract and configuration fields. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "The MiniDeepSeekMoE layer contract"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch08/eq-the-minideepseekmoe-layer-contract.tex` |
| Include command | `\input{equations/ch08/eq-the-minideepseekmoe-layer-contract}` |
| Label | `eq:ch08-the-minideepseekmoe-layer-contract` |
| Equation intent | y = x + shared_out + routed_out when the block is shown with residual context. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch08/lst-the-minideepseekmoe-layer-contract.tex` |
| Include command | `\input{listings/ch08/lst-the-minideepseekmoe-layer-contract}` |
| Label | `lst:ch08-the-minideepseekmoe-layer-contract` |
| Listing intent | MiniDeepSeekMoELayer __init__ showing router, routed experts, and shared experts. |

**Code objects to develop or reference:**
- `MiniDeepSeekMoELayer.forward contract`
...
2. Add only the minimal new object or function needed for **The MiniDeepSeekMoE layer contract**.
3. Write a CPU smoke test using the mini-example: Use D=128, E=4 routed experts, one shared expert, and K=2 in the smoke config.
...
**Expected code deliverable:** MiniDeepSeekMoELayer __init__ showing router, routed experts, and shared experts.
