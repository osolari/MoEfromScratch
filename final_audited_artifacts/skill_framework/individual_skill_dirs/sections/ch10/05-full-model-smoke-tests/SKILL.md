---
name: moe-ch10-s05-full-model-smoke-tests
description: use when developing section 10.5, full-model smoke tests, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 10.5 - Full-model smoke tests
...
Develop only Section 10.5 of *MoE Models from Scratch*: **Full-model smoke tests**.

- Chapter: 10. Assembling MiniDeepSeekMoE end to end
- Target section file: `latex_book_skeleton/chapters/ch10/sections/05-full-model-smoke-tests.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch10/fig-full-model-smoke-tests.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch10/tab-full-model-smoke-tests.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch10/eq-full-model-smoke-tests.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch10/lst-full-model-smoke-tests.tex`
- Required diagnostic: CI-style console summary for all smoke tests.
...
```text
Develop Section 10.5, "Full-model smoke tests", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 10.5 - Full-model smoke tests

Develop only Chapter 10.5, **Full-model smoke tests**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch10/05-full-model-smoke-tests.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch10/sections/05-full-model-smoke-tests.tex`
...
# Detailed section development brief: Chapter 10.5 - Full-model smoke tests
...
| Chapter | 10. Assembling MiniDeepSeekMoE end to end |
| Section | 10.5. Full-model smoke tests |
| Section label slug | `ch10-full-model-smoke-tests` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch10/sections/05-full-model-smoke-tests.tex` |
| Planned section include | `\input{chapters/ch10/sections/05-full-model-smoke-tests}` |
| Previous handoff | Chapter 10.4 - Training and sampling scripts |
| Next handoff | Chapter 10.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Run the smoke config on CPU for two forward/backward passes.
...
**This section's bridge sentence:** With a complete tested implementation, the next chapter runs experiments and interprets diagnostics.
...
1. **Opening paragraph.** State the concrete problem: Create a test suite that catches shape, routing, balancing, and serialization failures. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Run the smoke config on CPU for two forward/backward passes.
3. **Visual explanation.** Insert `\input{figures/ch10/fig-full-model-smoke-tests}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch10/eq-full-model-smoke-tests}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch10/lst-full-model-smoke-tests}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch10/tab-full-model-smoke-tests}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch10/fig-full-model-smoke-tests.tex` |
| Include command | `\input{figures/ch10/fig-full-model-smoke-tests}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch10-full-model-smoke-tests` |
| Caption intent | Smoke-test pipeline with pass/fail checkpoints after each stage. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Smoke-test pipeline with pass/fail checkpoints after each stage.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch10/tab-full-model-smoke-tests.tex` |
| Include command | `\input{tables/ch10/tab-full-model-smoke-tests}` |
| Label | `tab:ch10-full-model-smoke-tests` |
| Caption intent | Test names, purpose, and expected runtime tier. |
| Required columns | Config field | Default | Role | Allowed values | Used in |

**Planned table rows:**
- row 1: define the primary object for "Full-model smoke tests"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch10/eq-full-model-smoke-tests.tex` |
| Include command | `\input{equations/ch10/eq-full-model-smoke-tests}` |
| Label | `eq:ch10-full-model-smoke-tests` |
| Equation intent | No NaNs and finite loss invariant. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch10/lst-full-model-smoke-tests.tex` |
| Include command | `\input{listings/ch10/lst-full-model-smoke-tests}` |
| Label | `lst:ch10-full-model-smoke-tests` |
| Listing intent | tests/test_minideepseekmoe_smoke.py with forward, backward, bias-update, and save-load tests. |

**Code objects to develop or reference:**
- `pytest or script smoke tests`
- `CPU tiny config`
...
2. Add only the minimal new object or function needed for **Full-model smoke tests**.
3. Write a CPU smoke test using the mini-example: Run the smoke config on CPU for two forward/backward passes.
...
**Expected code deliverable:** tests/test_minideepseekmoe_smoke.py with forward, backward, bias-update, and save-load tests.
