---
name: moe-ch07-s04-router-score-scale-and-z-loss
description: use when developing section 7.4, router score scale and z-loss, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 7.4 - Router score scale and z-loss
...
Develop only Section 7.4 of *MoE Models from Scratch*: **Router score scale and z-loss**.

- Chapter: 7. Router training and load-balancing losses
- Target section file: `latex_book_skeleton/chapters/ch07/sections/04-router-score-scale-and-z-loss.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch07/fig-router-score-scale-and-z-loss.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch07/tab-router-score-scale-and-z-loss.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch07/eq-router-score-scale-and-z-loss.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch07/lst-router-score-scale-and-z-loss.tex`
- Required diagnostic: Track router score max, entropy, and z-loss over training.
...
```text
Develop Section 7.4, "Router score scale and z-loss", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 7.4 - Router score scale and z-loss

Develop only Chapter 7.4, **Router score scale and z-loss**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch07/04-router-score-scale-and-z-loss.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch07/sections/04-router-score-scale-and-z-loss.tex`
...
# Detailed section development brief: Chapter 7.4 - Router score scale and z-loss
...
| Chapter | 7. Router training and load-balancing losses |
| Section | 7.4. Router score scale and z-loss |
| Section label slug | `ch07-router-score-scale-and-z-loss` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch07/sections/04-router-score-scale-and-z-loss.tex` |
| Planned section include | `\input{chapters/ch07/sections/04-router-score-scale-and-z-loss}` |
| Previous handoff | Chapter 7.3 - Integrating auxiliary loss into training |
| Next handoff | Chapter 7.5 - Routing dashboards and training logs |
...
**Mini-example to anchor the section:** Compare a modest logit row with a very large logit row that creates near-hard routing.
...
**This section's bridge sentence:** The regularizers become useful only if the diagnostics are visible during training.
...
1. **Opening paragraph.** State the concrete problem: Introduce a simple regularizer for overly large router logits or scores. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Compare a modest logit row with a very large logit row that creates near-hard routing.
3. **Visual explanation.** Insert `\input{figures/ch07/fig-router-score-scale-and-z-loss}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch07/eq-router-score-scale-and-z-loss}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch07/lst-router-score-scale-and-z-loss}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch07/tab-router-score-scale-and-z-loss}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch07/fig-router-score-scale-and-z-loss.tex` |
| Include command | `\input{figures/ch07/fig-router-score-scale-and-z-loss}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch07-router-score-scale-and-z-loss` |
| Caption intent | Router logit scale affects probability sharpness before top-k selection. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Router logit scale affects probability sharpness before top-k selection.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch07/tab-router-score-scale-and-z-loss.tex` |
| Include command | `\input{tables/ch07/tab-router-score-scale-and-z-loss}` |
| Label | `tab:ch07-router-score-scale-and-z-loss` |
| Caption intent | Router regularization options: z-loss, entropy, noise, and clipping. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Router score scale and z-loss"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch07/eq-router-score-scale-and-z-loss.tex` |
| Include command | `\input{equations/ch07/eq-router-score-scale-and-z-loss}` |
| Label | `eq:ch07-router-score-scale-and-z-loss` |
| Equation intent | z_loss based on squared logsumexp of router logits. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch07/lst-router-score-scale-and-z-loss.tex` |
| Include command | `\input{listings/ch07/lst-router-score-scale-and-z-loss}` |
| Label | `lst:ch07-router-score-scale-and-z-loss` |
| Listing intent | router_z_loss function and optional coefficient in the training config. |
...
2. Add only the minimal new object or function needed for **Router score scale and z-loss**.
3. Write a CPU smoke test using the mini-example: Compare a modest logit row with a very large logit row that creates near-hard routing.
...
**Expected code deliverable:** router_z_loss function and optional coefficient in the training config.
