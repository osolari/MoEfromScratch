# Detailed section development brief: Chapter 2.3 - Causal self-attention as the context path
...
| Section | 2.3. Causal self-attention as the context path |
| Section label slug | `ch02-causal-self-attention-as-the-context-path` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch02/sections/03-causal-self-attention-as-the-context-path.tex` |
| Planned section include | `\input{chapters/ch02/sections/03-causal-self-attention-as-the-context-path}` |
| Previous handoff | Chapter 2.2 - Token and position embeddings |
| Next handoff | Chapter 2.4 - Dense feed-forward block as the replacement target |
...
**Mini-example to anchor the section:** Trace a single attention head with B=1, T=4, D=8.
...
**This section's bridge sentence:** After attention mixes context, the dense FFN transforms each token independently.
...
1. **Opening paragraph.** State the concrete problem: Build enough attention to make the baseline a real decoder while keeping MoE focus on the FFN. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Trace a single attention head with B=1, T=4, D=8.
3. **Visual explanation.** Insert `\input{figures/ch02/fig-causal-self-attention-as-the-context-path}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch02/eq-causal-self-attention-as-the-context-path}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch02/lst-causal-self-attention-as-the-context-path}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch02/tab-causal-self-attention-as-the-context-path}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch02/fig-causal-self-attention-as-the-context-path.tex` |
| Include command | `\input{figures/ch02/fig-causal-self-attention-as-the-context-path}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch02-causal-self-attention-as-the-context-path` |
| Caption intent | Causal mask applied to attention scores before softmax. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Causal mask applied to attention scores before softmax.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch02/tab-causal-self-attention-as-the-context-path.tex` |
| Include command | `\input{tables/ch02/tab-causal-self-attention-as-the-context-path}` |
| Label | `tab:ch02-causal-self-attention-as-the-context-path` |
| Caption intent | Q, K, V, score, probability, and output shapes. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Causal self-attention as the context path"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch02/eq-causal-self-attention-as-the-context-path.tex` |
| Include command | `\input{equations/ch02/eq-causal-self-attention-as-the-context-path}` |
| Label | `eq:ch02-causal-self-attention-as-the-context-path` |
| Equation intent | Masked attention softmax(QK^T / sqrt(d_head))V. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch02/lst-causal-self-attention-as-the-context-path.tex` |
| Include command | `\input{listings/ch02/lst-causal-self-attention-as-the-context-path}` |
| Label | `lst:ch02-causal-self-attention-as-the-context-path` |
| Listing intent | Compact multi-head causal attention module. |
...
2. Add only the minimal new object or function needed for **Causal self-attention as the context path**.
3. Write a CPU smoke test using the mini-example: Trace a single attention head with B=1, T=4, D=8.
...
**Expected code deliverable:** Compact multi-head causal attention module.
