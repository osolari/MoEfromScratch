# Detailed section development brief: Chapter 8.5 - Specialization diagnostics
...
| Section | 8.5. Specialization diagnostics |
| Section label slug | `ch08-specialization-diagnostics` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch08/sections/05-specialization-diagnostics.tex` |
| Planned section include | `\input{chapters/ch08/sections/05-specialization-diagnostics}` |
| Previous handoff | Chapter 8.4 - Combining shared and routed outputs |
| Next handoff | Chapter 8.6 - Chapter summary and handoff |
...
**Mini-example to anchor the section:** Group tokens by simple categories or positions and inspect selected expert frequencies.
...
**This section's bridge sentence:** The layer architecture is now complete; the next chapter replaces auxiliary loss with dynamic router bias balancing.
...
1. **Opening paragraph.** State the concrete problem: Plan diagnostics that reveal whether routed experts are used differently across tokens. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Group tokens by simple categories or positions and inspect selected expert frequencies.
3. **Visual explanation.** Insert `\input{figures/ch08/fig-specialization-diagnostics}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch08/eq-specialization-diagnostics}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch08/lst-specialization-diagnostics}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch08/tab-specialization-diagnostics}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch08/fig-specialization-diagnostics.tex` |
| Include command | `\input{figures/ch08/fig-specialization-diagnostics}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch08-specialization-diagnostics` |
| Caption intent | Expert usage heatmap by token group or position bucket. |
...
**Figure content plan:** Expert usage heatmap by token group or position bucket.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch08/tab-specialization-diagnostics.tex` |
| Include command | `\input{tables/ch08/tab-specialization-diagnostics}` |
| Label | `tab:ch08-specialization-diagnostics` |
| Caption intent | Specialization probes: token string, position, loss contribution, selected experts. |
| Required columns | Metric or run | What it measures | Expected healthy pattern | Warning pattern | Where logged |

**Planned table rows:**
- row 1: define the primary object for "Specialization diagnostics"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch08/eq-specialization-diagnostics.tex` |
| Include command | `\input{equations/ch08/eq-specialization-diagnostics}` |
| Label | `eq:ch08-specialization-diagnostics` |
| Equation intent | Conditional usage frequency p(expert | token_group). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch08/lst-specialization-diagnostics.tex` |
| Include command | `\input{listings/ch08/lst-specialization-diagnostics}` |
| Label | `lst:ch08-specialization-diagnostics` |
| Listing intent | collect_expert_usage_by_token helper. |
...
2. Add only the minimal new object or function needed for **Specialization diagnostics**.
3. Write a CPU smoke test using the mini-example: Group tokens by simple categories or positions and inspect selected expert frequencies.
...
**Expected code deliverable:** collect_expert_usage_by_token helper.
