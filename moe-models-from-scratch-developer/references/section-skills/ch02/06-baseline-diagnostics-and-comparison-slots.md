---
name: moe-ch02-s06-baseline-diagnostics-and-comparison-slots
description: use when developing section 2.6, baseline diagnostics and comparison slots, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 2.6 - Baseline diagnostics and comparison slots
...
Develop only Section 2.6 of *MoE Models from Scratch*: **Baseline diagnostics and comparison slots**.

- Chapter: 2. A dense Transformer baseline from scratch
- Target section file: `latex_book_skeleton/chapters/ch02/sections/06-baseline-diagnostics-and-comparison-slots.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch02/fig-baseline-diagnostics-and-comparison-slots.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch02/tab-baseline-diagnostics-and-comparison-slots.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch02/eq-baseline-diagnostics-and-comparison-slots.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch02/lst-baseline-diagnostics-and-comparison-slots.tex`
- Required diagnostic: Write the first baseline row in the experiment comparison table.
...
```text
Develop Section 2.6, "Baseline diagnostics and comparison slots", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 2.6 - Baseline diagnostics and comparison slots

Develop only Chapter 2.6, **Baseline diagnostics and comparison slots**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch02/06-baseline-diagnostics-and-comparison-slots.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch02/sections/06-baseline-diagnostics-and-comparison-slots.tex`
...
# Detailed section development brief: Chapter 2.6 - Baseline diagnostics and comparison slots
...
| Chapter | 2. A dense Transformer baseline from scratch |
| Section | 2.6. Baseline diagnostics and comparison slots |
| Section label slug | `ch02-baseline-diagnostics-and-comparison-slots` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch02/sections/06-baseline-diagnostics-and-comparison-slots.tex` |
| Planned section include | `\input{chapters/ch02/sections/06-baseline-diagnostics-and-comparison-slots}` |
| Previous handoff | Chapter 2.5 - Training loop and sampling baseline |
| Next handoff | Chapter 3.1 - Expert MLPs as named FFNs |
...
**Mini-example to anchor the section:** Create a metrics dictionary with loss, tokens/sec, parameter count, and activation estimate.
...
**This section's bridge sentence:** The next chapter replaces the dense FFN with a small expert bank and a router.
...
1. **Opening paragraph.** State the concrete problem: Prepare diagnostic hooks that will later compare dense and sparse models fairly. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Create a metrics dictionary with loss, tokens/sec, parameter count, and activation estimate.
3. **Visual explanation.** Insert `\input{figures/ch02/fig-baseline-diagnostics-and-comparison-slots}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch02/eq-baseline-diagnostics-and-comparison-slots}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch02/lst-baseline-diagnostics-and-comparison-slots}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch02/tab-baseline-diagnostics-and-comparison-slots}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch02/fig-baseline-diagnostics-and-comparison-slots.tex` |
| Include command | `\input{figures/ch02/fig-baseline-diagnostics-and-comparison-slots}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch02-baseline-diagnostics-and-comparison-slots` |
| Caption intent | Dashboard placeholder for loss curves, parameter counts, and future routing metrics. |
...
**Figure content plan:** Dashboard placeholder for loss curves, parameter counts, and future routing metrics.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch02/tab-baseline-diagnostics-and-comparison-slots.tex` |
| Include command | `\input{tables/ch02/tab-baseline-diagnostics-and-comparison-slots}` |
| Label | `tab:ch02-baseline-diagnostics-and-comparison-slots` |
| Caption intent | Metrics available now versus metrics added by MoE layers. |
| Required columns | Metric or run | What it measures | Expected healthy pattern | Warning pattern | Where logged |

**Planned table rows:**
- row 1: define the primary object for "Baseline diagnostics and comparison slots"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch02/eq-baseline-diagnostics-and-comparison-slots.tex` |
| Include command | `\input{equations/ch02/eq-baseline-diagnostics-and-comparison-slots}` |
| Label | `eq:ch02-baseline-diagnostics-and-comparison-slots` |
| Equation intent | Perplexity = exp(validation cross-entropy). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch02/lst-baseline-diagnostics-and-comparison-slots.tex` |
| Include command | `\input{listings/ch02/lst-baseline-diagnostics-and-comparison-slots}` |
| Label | `lst:ch02-baseline-diagnostics-and-comparison-slots` |
| Listing intent | count_parameters and estimate_active_parameters helper functions. |

**Code objects to develop or reference:**
- `baseline diagnostics collector`
...
2. Add only the minimal new object or function needed for **Baseline diagnostics and comparison slots**.
3. Write a CPU smoke test using the mini-example: Create a metrics dictionary with loss, tokens/sec, parameter count, and activation estimate.
...
**Expected code deliverable:** count_parameters and estimate_active_parameters helper functions.
