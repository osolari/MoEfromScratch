---
name: moe-ch02-s01-dataset-and-token-batches
description: use when developing section 2.1, dataset and token batches, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 2.1 - Dataset and token batches
...
Develop only Section 2.1 of *MoE Models from Scratch*: **Dataset and token batches**.

- Chapter: 2. A dense Transformer baseline from scratch
- Target section file: `latex_book_skeleton/chapters/ch02/sections/01-dataset-and-token-batches.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch02/fig-dataset-and-token-batches.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch02/tab-dataset-and-token-batches.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch02/eq-dataset-and-token-batches.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch02/lst-dataset-and-token-batches.tex`
- Required diagnostic: Print the first input-target pair and decode it to verify shifting.
...
```text
Develop Section 2.1, "Dataset and token batches", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 2.1 - Dataset and token batches

Develop only Chapter 2.1, **Dataset and token batches**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch02/01-dataset-and-token-batches.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch02/sections/01-dataset-and-token-batches.tex`
...
# Detailed section development brief: Chapter 2.1 - Dataset and token batches
...
| Chapter | 2. A dense Transformer baseline from scratch |
| Section | 2.1. Dataset and token batches |
| Section label slug | `ch02-dataset-and-token-batches` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch02/sections/01-dataset-and-token-batches.tex` |
| Planned section include | `\input{chapters/ch02/sections/01-dataset-and-token-batches}` |
| Previous handoff | Chapter 1.6 - Chapter summary and handoff |
| Next handoff | Chapter 2.2 - Token and position embeddings |
...
**Mini-example to anchor the section:** Load a short text corpus, tokenize it, and create B=2, T=8 training examples.
...
**This section's bridge sentence:** Token IDs become vectors through embeddings and positions.
...
1. **Opening paragraph.** State the concrete problem: Create the small next-token prediction pipeline used for every model variant. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Load a short text corpus, tokenize it, and create B=2, T=8 training examples.
3. **Visual explanation.** Insert `\input{figures/ch02/fig-dataset-and-token-batches}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch02/eq-dataset-and-token-batches}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch02/lst-dataset-and-token-batches}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch02/tab-dataset-and-token-batches}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch02/fig-dataset-and-token-batches.tex` |
| Include command | `\input{figures/ch02/fig-dataset-and-token-batches}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch02-dataset-and-token-batches` |
| Caption intent | Text to token IDs to input-target shifted batches. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Text to token IDs to input-target shifted batches.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch02/tab-dataset-and-token-batches.tex` |
| Include command | `\input{tables/ch02/tab-dataset-and-token-batches}` |
| Label | `tab:ch02-dataset-and-token-batches` |
| Caption intent | Batch tensor shapes and dataloader responsibilities. |
| Required columns | Tensor name | Shape | Created by | Used by | Failure check |

**Planned table rows:**
- row 1: define the primary object for "Dataset and token batches"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch02/eq-dataset-and-token-batches.tex` |
| Include command | `\input{equations/ch02/eq-dataset-and-token-batches}` |
| Label | `eq:ch02-dataset-and-token-batches` |
| Equation intent | Next-token objective index relation target[t] = input[t+1]. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch02/lst-dataset-and-token-batches.tex` |
| Include command | `\input{listings/ch02/lst-dataset-and-token-batches}` |
| Label | `lst:ch02-dataset-and-token-batches` |
| Listing intent | Minimal get_batch function with input and target shifts. |
...
2. Add only the minimal new object or function needed for **Dataset and token batches**.
3. Write a CPU smoke test using the mini-example: Load a short text corpus, tokenize it, and create B=2, T=8 training examples.
...
**Expected code deliverable:** Minimal get_batch function with input and target shifts.
