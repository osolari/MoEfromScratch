---
name: moe-ch12-s04-expert-parallelism-and-all-to-all-communic
description: use when developing section 12.4, expert parallelism and all-to-all communication, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 12.4 - Expert parallelism and all-to-all communication
...
Develop only Section 12.4 of *MoE Models from Scratch*: **Expert parallelism and all-to-all communication**.

- Chapter: 12. Inference, performance, and scaling beyond the toy model
- Target section file: `latex_book_skeleton/chapters/ch12/sections/04-expert-parallelism-and-all-to-all-communication.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch12/fig-expert-parallelism-and-all-to-all-communication.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch12/tab-expert-parallelism-and-all-to-all-communication.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch12/eq-expert-parallelism-and-all-to-all-communication.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch12/lst-expert-parallelism-and-all-to-all-communication.tex`
- Required diagnostic: Thought experiment table estimating routed activation traffic for a small config.
...
```text
Develop Section 12.4, "Expert parallelism and all-to-all communication", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 12.4 - Expert parallelism and all-to-all communication

Develop only Chapter 12.4, **Expert parallelism and all-to-all communication**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch12/04-expert-parallelism-and-all-to-all-communication.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch12/sections/04-expert-parallelism-and-all-to-all-communication.tex`
...
# Detailed section development brief: Chapter 12.4 - Expert parallelism and all-to-all communication
...
| Chapter | 12. Inference, performance, and scaling beyond the toy model |
| Section | 12.4. Expert parallelism and all-to-all communication |
| Section label slug | `ch12-expert-parallelism-and-all-to-all-communication` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch12/sections/04-expert-parallelism-and-all-to-all-communication.tex` |
| Planned section include | `\input{chapters/ch12/sections/04-expert-parallelism-and-all-to-all-communication}` |
| Previous handoff | Chapter 12.3 - Compute, memory, and active-parameter accounting |
| Next handoff | Chapter 12.5 - Mapping the book model to production MoE families |
...
**Mini-example to anchor the section:** Two devices each host two experts; tokens must move to the device containing their selected experts.
...
**This section's bridge sentence:** This systems view helps map the toy model to public large-MoE architecture descriptions.
...
1. **Opening paragraph.** State the concrete problem: Explain the systems picture conceptually while keeping implementation out of scope. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Two devices each host two experts; tokens must move to the device containing their selected experts.
3. **Visual explanation.** Insert `\input{figures/ch12/fig-expert-parallelism-and-all-to-all-communication}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch12/eq-expert-parallelism-and-all-to-all-communication}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch12/lst-expert-parallelism-and-all-to-all-communication}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch12/tab-expert-parallelism-and-all-to-all-communication}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch12/fig-expert-parallelism-and-all-to-all-communication.tex` |
| Include command | `\input{figures/ch12/fig-expert-parallelism-and-all-to-all-communication}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch12-expert-parallelism-and-all-to-all-communication` |
| Caption intent | Tokens routed across devices through an all-to-all exchange, expert compute, and return exchange. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Tokens routed across devices through an all-to-all exchange, expert compute, and return exchange.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch12/tab-expert-parallelism-and-all-to-all-communication.tex` |
| Include command | `\input{tables/ch12/tab-expert-parallelism-and-all-to-all-communication}` |
| Label | `tab:ch12-expert-parallelism-and-all-to-all-communication` |
| Caption intent | Single-device MoE versus expert-parallel MoE responsibilities. |
| Required columns | Quantity | Dense baseline | MoE path | Caveat | Measurement |

**Planned table rows:**
- row 1: define the primary object for "Expert parallelism and all-to-all communication"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch12/eq-expert-parallelism-and-all-to-all-communication.tex` |
| Include command | `\input{equations/ch12/eq-expert-parallelism-and-all-to-all-communication}` |
| Label | `eq:ch12-expert-parallelism-and-all-to-all-communication` |
| Equation intent | Communication volume depends on routed token count, D, K, and bytes per element. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch12/lst-expert-parallelism-and-all-to-all-communication.tex` |
| Include command | `\input{listings/ch12/lst-expert-parallelism-and-all-to-all-communication}` |
| Label | `lst:ch12-expert-parallelism-and-all-to-all-communication` |
| Listing intent | Conceptual pseudocode for distributed dispatch without runnable distributed code. |

**Code objects to develop or reference:**
- `expert_parallelism_schematic data`
- `all_to_all toy simulator`
...
2. Add only the minimal new object or function needed for **Expert parallelism and all-to-all communication**.
3. Write a CPU smoke test using the mini-example: Two devices each host two experts; tokens must move to the device containing their selected experts.
...
**Expected code deliverable:** Conceptual pseudocode for distributed dispatch without runnable distributed code.
