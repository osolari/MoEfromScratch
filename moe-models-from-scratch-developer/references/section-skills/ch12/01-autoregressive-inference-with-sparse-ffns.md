---
name: moe-ch12-s01-autoregressive-inference-with-sparse-ffns
description: use when developing section 12.1, autoregressive inference with sparse ffns, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 12.1 - Autoregressive inference with sparse FFNs
...
Develop only Section 12.1 of *MoE Models from Scratch*: **Autoregressive inference with sparse FFNs**.

- Chapter: 12. Inference, performance, and scaling beyond the toy model
- Target section file: `latex_book_skeleton/chapters/ch12/sections/01-autoregressive-inference-with-sparse-ffns.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch12/fig-autoregressive-inference-with-sparse-ffns.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch12/tab-autoregressive-inference-with-sparse-ffns.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch12/eq-autoregressive-inference-with-sparse-ffns.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch12/lst-autoregressive-inference-with-sparse-ffns.tex`
- Required diagnostic: Print selected experts for each generated token in a short sample.
...
```text
Develop Section 12.1, "Autoregressive inference with sparse FFNs", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 12.1 - Autoregressive inference with sparse FFNs

Develop only Chapter 12.1, **Autoregressive inference with sparse FFNs**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch12/01-autoregressive-inference-with-sparse-ffns.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch12/sections/01-autoregressive-inference-with-sparse-ffns.tex`
...
# Detailed section development brief: Chapter 12.1 - Autoregressive inference with sparse FFNs
...
| Chapter | 12. Inference, performance, and scaling beyond the toy model |
| Section | 12.1. Autoregressive inference with sparse FFNs |
| Section label slug | `ch12-autoregressive-inference-with-sparse-ffns` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch12/sections/01-autoregressive-inference-with-sparse-ffns.tex` |
| Planned section include | `\input{chapters/ch12/sections/01-autoregressive-inference-with-sparse-ffns}` |
| Previous handoff | Chapter 11.6 - Chapter summary and handoff |
| Next handoff | Chapter 12.2 - Batching routed tokens at inference |
...
**Mini-example to anchor the section:** Generate one new token and trace attention, router, selected experts, and logits.
...
**This section's bridge sentence:** Batching multiple requests complicates the expert batches.
...
1. **Opening paragraph.** State the concrete problem: Explain what changes and what stays the same when a trained MoE model generates text one token at a time. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Generate one new token and trace attention, router, selected experts, and logits.
3. **Visual explanation.** Insert `\input{figures/ch12/fig-autoregressive-inference-with-sparse-ffns}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch12/eq-autoregressive-inference-with-sparse-ffns}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch12/lst-autoregressive-inference-with-sparse-ffns}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch12/tab-autoregressive-inference-with-sparse-ffns}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch12/fig-autoregressive-inference-with-sparse-ffns.tex` |
| Include command | `\input{figures/ch12/fig-autoregressive-inference-with-sparse-ffns}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch12-autoregressive-inference-with-sparse-ffns` |
| Caption intent | One-token inference path through attention cache and MoE layer. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** One-token inference path through attention cache and MoE layer.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch12/tab-autoregressive-inference-with-sparse-ffns.tex` |
| Include command | `\input{tables/ch12/tab-autoregressive-inference-with-sparse-ffns}` |
| Label | `tab:ch12-autoregressive-inference-with-sparse-ffns` |
| Caption intent | Training-time versus inference-time routing tensors. |
| Required columns | Quantity | Dense baseline | MoE path | Caveat | Measurement |

**Planned table rows:**
- row 1: define the primary object for "Autoregressive inference with sparse FFNs"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch12/eq-autoregressive-inference-with-sparse-ffns.tex` |
| Include command | `\input{equations/ch12/eq-autoregressive-inference-with-sparse-ffns}` |
| Label | `eq:ch12-autoregressive-inference-with-sparse-ffns` |
| Equation intent | Per-step active expert calls equal layers_with_moe * K for one token. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch12/lst-autoregressive-inference-with-sparse-ffns.tex` |
| Include command | `\input{listings/ch12/lst-autoregressive-inference-with-sparse-ffns}` |
| Label | `lst:ch12-autoregressive-inference-with-sparse-ffns` |
| Listing intent | generate function with routing metrics optionally returned per step. |

**Code objects to develop or reference:**
- `generate.py sparse path notes`
...
2. Add only the minimal new object or function needed for **Autoregressive inference with sparse FFNs**.
3. Write a CPU smoke test using the mini-example: Generate one new token and trace attention, router, selected experts, and logits.
...
**Expected code deliverable:** generate function with routing metrics optionally returned per step.
