---
name: moe-ch12-s02-batching-routed-tokens-at-inference
description: use when developing section 12.2, batching routed tokens at inference, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 12.2 - Batching routed tokens at inference
...
Develop only Section 12.2 of *MoE Models from Scratch*: **Batching routed tokens at inference**.

- Chapter: 12. Inference, performance, and scaling beyond the toy model
- Target section file: `latex_book_skeleton/chapters/ch12/sections/02-batching-routed-tokens-at-inference.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch12/fig-batching-routed-tokens-at-inference.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch12/tab-batching-routed-tokens-at-inference.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch12/eq-batching-routed-tokens-at-inference.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch12/lst-batching-routed-tokens-at-inference.tex`
- Required diagnostic: Histogram expert batch sizes over a generated sequence.
...
```text
Develop Section 12.2, "Batching routed tokens at inference", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 12.2 - Batching routed tokens at inference

Develop only Chapter 12.2, **Batching routed tokens at inference**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch12/02-batching-routed-tokens-at-inference.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch12/sections/02-batching-routed-tokens-at-inference.tex`
...
# Detailed section development brief: Chapter 12.2 - Batching routed tokens at inference
...
| Chapter | 12. Inference, performance, and scaling beyond the toy model |
| Section | 12.2. Batching routed tokens at inference |
| Section label slug | `ch12-batching-routed-tokens-at-inference` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch12/sections/02-batching-routed-tokens-at-inference.tex` |
| Planned section include | `\input{chapters/ch12/sections/02-batching-routed-tokens-at-inference}` |
| Previous handoff | Chapter 12.1 - Autoregressive inference with sparse FFNs |
| Next handoff | Chapter 12.3 - Compute, memory, and active-parameter accounting |
...
**Mini-example to anchor the section:** Two prompts in a batch route their next token to different experts.
...
**This section's bridge sentence:** Compute and memory estimates clarify where the toy implementation stops scaling.
...
1. **Opening paragraph.** State the concrete problem: Show why MoE inference wants tokens for the same expert to be grouped efficiently. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Two prompts in a batch route their next token to different experts.
3. **Visual explanation.** Insert `\input{figures/ch12/fig-batching-routed-tokens-at-inference}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch12/eq-batching-routed-tokens-at-inference}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch12/lst-batching-routed-tokens-at-inference}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch12/tab-batching-routed-tokens-at-inference}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch12/fig-batching-routed-tokens-at-inference.tex` |
| Include command | `\input{figures/ch12/fig-batching-routed-tokens-at-inference}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch12-batching-routed-tokens-at-inference` |
| Caption intent | Inference batch tokens packed into expert-specific microbatches. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Inference batch tokens packed into expert-specific microbatches.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch12/tab-batching-routed-tokens-at-inference.tex` |
| Include command | `\input{tables/ch12/tab-batching-routed-tokens-at-inference}` |
| Label | `tab:ch12-batching-routed-tokens-at-inference` |
| Caption intent | Inference batching challenges: uneven loads, small expert batches, padding, latency. |
| Required columns | Tensor name | Shape | Created by | Used by | Failure check |

**Planned table rows:**
- row 1: define the primary object for "Batching routed tokens at inference"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch12/eq-batching-routed-tokens-at-inference.tex` |
| Include command | `\input{equations/ch12/eq-batching-routed-tokens-at-inference}` |
| Label | `eq:ch12-batching-routed-tokens-at-inference` |
| Equation intent | Expert batch size distribution at generation step s. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch12/lst-batching-routed-tokens-at-inference.tex` |
| Include command | `\input{listings/ch12/lst-batching-routed-tokens-at-inference}` |
| Label | `lst:ch12-batching-routed-tokens-at-inference` |
| Listing intent | profile_inference_routes function collecting per-step expert batch sizes. |

**Code objects to develop or reference:**
- `inference_batch_router`
- `per-step expert batching demo`
...
2. Add only the minimal new object or function needed for **Batching routed tokens at inference**.
3. Write a CPU smoke test using the mini-example: Two prompts in a batch route their next token to different experts.
...
**Expected code deliverable:** profile_inference_routes function collecting per-step expert batch sizes.
