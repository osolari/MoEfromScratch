---
name: moe-ch05-s03-dispatch-for-two-expert-paths-per-token
description: use when developing section 5.3, dispatch for two expert paths per token, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 5.3 - Dispatch for two expert paths per token
...
Develop only Section 5.3 of *MoE Models from Scratch*: **Dispatch for two expert paths per token**.

- Chapter: 5. Top-2 routing and weighted expert combination
- Target section file: `latex_book_skeleton/chapters/ch05/sections/03-dispatch-for-two-expert-paths-per-token.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch05/fig-dispatch-for-two-expert-paths-per-token.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch05/tab-dispatch-for-two-expert-paths-per-token.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch05/eq-dispatch-for-two-expert-paths-per-token.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch05/lst-dispatch-for-two-expert-paths-per-token.tex`
- Required diagnostic: Check output changes when gates are manually swapped.
...
```text
Develop Section 5.3, "Dispatch for two expert paths per token", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 5.3 - Dispatch for two expert paths per token

Develop only Chapter 5.3, **Dispatch for two expert paths per token**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch05/03-dispatch-for-two-expert-paths-per-token.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch05/sections/03-dispatch-for-two-expert-paths-per-token.tex`
...
# Detailed section development brief: Chapter 5.3 - Dispatch for two expert paths per token
...
| Chapter | 5. Top-2 routing and weighted expert combination |
| Section | 5.3. Dispatch for two expert paths per token |
| Section label slug | `ch05-dispatch-for-two-expert-paths-per-token` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch05/sections/03-dispatch-for-two-expert-paths-per-token.tex` |
| Planned section include | `\input{chapters/ch05/sections/03-dispatch-for-two-expert-paths-per-token}` |
| Previous handoff | Chapter 5.2 - Normalizing selected router scores |
| Next handoff | Chapter 5.4 - Comparing top-1 and top-2 diagnostics |
...
**Mini-example to anchor the section:** Token 0 goes to experts 1 and 3; both outputs are accumulated into output[0].
...
**This section's bridge sentence:** Top-2 dispatch is correct but still slow; vectorization comes after the routing math is clear.
...
1. **Opening paragraph.** State the concrete problem: Adapt the dispatch loop so each selected expert path contributes to the same output row. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Token 0 goes to experts 1 and 3; both outputs are accumulated into output[0].
3. **Visual explanation.** Insert `\input{figures/ch05/fig-dispatch-for-two-expert-paths-per-token}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch05/eq-dispatch-for-two-expert-paths-per-token}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch05/lst-dispatch-for-two-expert-paths-per-token}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch05/tab-dispatch-for-two-expert-paths-per-token}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch05/fig-dispatch-for-two-expert-paths-per-token.tex` |
| Include command | `\input{figures/ch05/fig-dispatch-for-two-expert-paths-per-token}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch05-dispatch-for-two-expert-paths-per-token` |
| Caption intent | Two paths per token gathered into expert batches and accumulated back. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Two paths per token gathered into expert batches and accumulated back.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch05/tab-dispatch-for-two-expert-paths-per-token.tex` |
| Include command | `\input{tables/ch05/tab-dispatch-for-two-expert-paths-per-token}` |
| Label | `tab:ch05-dispatch-for-two-expert-paths-per-token` |
| Caption intent | Data structures for topk_idx, gates, expert batches, and output accumulation. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Dispatch for two expert paths per token"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch05/eq-dispatch-for-two-expert-paths-per-token.tex` |
| Include command | `\input{equations/ch05/eq-dispatch-for-two-expert-paths-per-token}` |
| Label | `eq:ch05-dispatch-for-two-expert-paths-per-token` |
| Equation intent | out_t += gate_{tj} expert_{idx_{tj}}(x_t). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch05/lst-dispatch-for-two-expert-paths-per-token.tex` |
| Include command | `\input{listings/ch05/lst-dispatch-for-two-expert-paths-per-token}` |
| Label | `lst:ch05-dispatch-for-two-expert-paths-per-token` |
| Listing intent | Top2MoELayer forward pass with nested selected-expert accumulation. |

**Code objects to develop or reference:**
- `Top2MoELayer.dispatch_records`
- `weighted combine`
...
2. Add only the minimal new object or function needed for **Dispatch for two expert paths per token**.
3. Write a CPU smoke test using the mini-example: Token 0 goes to experts 1 and 3; both outputs are accumulated into output[0].
...
**Expected code deliverable:** Top2MoELayer forward pass with nested selected-expert accumulation.
