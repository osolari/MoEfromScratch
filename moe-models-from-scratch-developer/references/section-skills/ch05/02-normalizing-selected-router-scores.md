---
name: moe-ch05-s02-normalizing-selected-router-scores
description: use when developing section 5.2, normalizing selected router scores, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 5.2 - Normalizing selected router scores
...
Develop only Section 5.2 of *MoE Models from Scratch*: **Normalizing selected router scores**.

- Chapter: 5. Top-2 routing and weighted expert combination
- Target section file: `latex_book_skeleton/chapters/ch05/sections/02-normalizing-selected-router-scores.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch05/fig-normalizing-selected-router-scores.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch05/tab-normalizing-selected-router-scores.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch05/eq-normalizing-selected-router-scores.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch05/lst-normalizing-selected-router-scores.tex`
- Required diagnostic: Assert torch.allclose(gates.sum(-1), ones).
...
```text
Develop Section 5.2, "Normalizing selected router scores", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 5.2 - Normalizing selected router scores

Develop only Chapter 5.2, **Normalizing selected router scores**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch05/02-normalizing-selected-router-scores.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch05/sections/02-normalizing-selected-router-scores.tex`
...
# Detailed section development brief: Chapter 5.2 - Normalizing selected router scores
...
| Chapter | 5. Top-2 routing and weighted expert combination |
| Section | 5.2. Normalizing selected router scores |
| Section label slug | `ch05-normalizing-selected-router-scores` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch05/sections/02-normalizing-selected-router-scores.tex` |
| Planned section include | `\input{chapters/ch05/sections/02-normalizing-selected-router-scores}` |
| Previous handoff | Chapter 5.1 - Why top-2 changes the routing story |
| Next handoff | Chapter 5.3 - Dispatch for two expert paths per token |
...
**Mini-example to anchor the section:** Normalize selected scores [0.8, 0.2] and [0.51, 0.49] and compare combines.
...
**This section's bridge sentence:** The normalized gates weight two expert outputs per token.
...
1. **Opening paragraph.** State the concrete problem: Turn top-2 scores into gates that sum to one for each token. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Normalize selected scores [0.8, 0.2] and [0.51, 0.49] and compare combines.
3. **Visual explanation.** Insert `\input{figures/ch05/fig-normalizing-selected-router-scores}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch05/eq-normalizing-selected-router-scores}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch05/lst-normalizing-selected-router-scores}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch05/tab-normalizing-selected-router-scores}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch05/fig-normalizing-selected-router-scores.tex` |
| Include command | `\input{figures/ch05/fig-normalizing-selected-router-scores}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch05-normalizing-selected-router-scores` |
| Caption intent | Top-2 score row transformed into a two-weight gate vector. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Top-2 score row transformed into a two-weight gate vector.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch05/tab-normalizing-selected-router-scores.tex` |
| Include command | `\input{tables/ch05/tab-normalizing-selected-router-scores}` |
| Label | `tab:ch05-normalizing-selected-router-scores` |
| Caption intent | Raw scores, selected scores, normalized gates, and gate sums. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Normalizing selected router scores"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch05/eq-normalizing-selected-router-scores.tex` |
| Include command | `\input{equations/ch05/eq-normalizing-selected-router-scores}` |
| Label | `eq:ch05-normalizing-selected-router-scores` |
| Equation intent | gate_{tj} = score_{t,idx_{tj}} / sum_l score_{t,idx_{tl}}. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch05/lst-normalizing-selected-router-scores.tex` |
| Include command | `\input{listings/ch05/lst-normalizing-selected-router-scores}` |
| Label | `lst:ch05-normalizing-selected-router-scores` |
| Listing intent | selected_scores / selected_scores.sum(dim=-1, keepdim=True). |

**Code objects to develop or reference:**
- `selected_gate_normalize`
- `sum-to-one check`
...
2. Add only the minimal new object or function needed for **Normalizing selected router scores**.
3. Write a CPU smoke test using the mini-example: Normalize selected scores [0.8, 0.2] and [0.51, 0.49] and compare combines.
...
**Expected code deliverable:** selected_scores / selected_scores.sum(dim=-1, keepdim=True).
