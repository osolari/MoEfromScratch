---
name: moe-ch07-s02-auxiliary-load-balancing-loss
description: use when developing section 7.2, auxiliary load-balancing loss, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 7.2 - Auxiliary load-balancing loss
...
Develop only Section 7.2 of *MoE Models from Scratch*: **Auxiliary load-balancing loss**.

- Chapter: 7. Router training and load-balancing losses
- Target section file: `latex_book_skeleton/chapters/ch07/sections/02-auxiliary-load-balancing-loss.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch07/fig-auxiliary-load-balancing-loss.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch07/tab-auxiliary-load-balancing-loss.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch07/eq-auxiliary-load-balancing-loss.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch07/lst-auxiliary-load-balancing-loss.tex`
- Required diagnostic: Check loss is lower for balanced synthetic routing than collapsed routing.
...
```text
Develop Section 7.2, "Auxiliary load-balancing loss", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 7.2 - Auxiliary load-balancing loss
...
| Chapter | 7. Router training and load-balancing losses |
| Section | 7.2. Auxiliary load-balancing loss |
| Section label slug | `ch07-auxiliary-load-balancing-loss` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch07/sections/02-auxiliary-load-balancing-loss.tex` |
| Planned section include | `\input{chapters/ch07/sections/02-auxiliary-load-balancing-loss}` |
| Previous handoff | Chapter 7.1 - Expert imbalance as a training problem |
| Next handoff | Chapter 7.3 - Integrating auxiliary loss into training |
...
**Mini-example to anchor the section:** Compute expert load fractions and probability fractions for E=4 experts.
...
**This section's bridge sentence:** Auxiliary loss enters the total training objective with a coefficient.
...
1. **Opening paragraph.** State the concrete problem: Implement the classic balancing baseline as a transparent, measurable loss term. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Compute expert load fractions and probability fractions for E=4 experts.
3. **Visual explanation.** Insert `\input{figures/ch07/fig-auxiliary-load-balancing-loss}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch07/eq-auxiliary-load-balancing-loss}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch07/lst-auxiliary-load-balancing-loss}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch07/tab-auxiliary-load-balancing-loss}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch07/fig-auxiliary-load-balancing-loss.tex` |
| Include command | `\input{figures/ch07/fig-auxiliary-load-balancing-loss}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch07-auxiliary-load-balancing-loss` |
| Caption intent | Two bars per expert: selected load and router probability mass. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Two bars per expert: selected load and router probability mass.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch07/tab-auxiliary-load-balancing-loss.tex` |
| Include command | `\input{tables/ch07/tab-auxiliary-load-balancing-loss}` |
| Label | `tab:ch07-auxiliary-load-balancing-loss` |
| Caption intent | Inputs and outputs of the auxiliary loss function. |
| Required columns | Loss/term | Formula input | Gradient target | Weight/default | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Auxiliary load-balancing loss"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch07/eq-auxiliary-load-balancing-loss.tex` |
| Include command | `\input{equations/ch07/eq-auxiliary-load-balancing-loss}` |
| Label | `eq:ch07-auxiliary-load-balancing-loss` |
| Equation intent | Auxiliary loss proportional to E * sum_i load_i * prob_i. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch07/lst-auxiliary-load-balancing-loss.tex` |
| Include command | `\input{listings/ch07/lst-auxiliary-load-balancing-loss}` |
| Label | `lst:ch07-auxiliary-load-balancing-loss` |
| Listing intent | load_balancing_loss function returning scalar aux_loss and metrics. |
...
2. Add only the minimal new object or function needed for **Auxiliary load-balancing loss**.
3. Write a CPU smoke test using the mini-example: Compute expert load fractions and probability fractions for E=4 experts.
...
**Expected code deliverable:** load_balancing_loss function returning scalar aux_loss and metrics.
