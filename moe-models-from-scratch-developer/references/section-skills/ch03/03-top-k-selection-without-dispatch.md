---
name: moe-ch03-s03-top-k-selection-without-dispatch
description: use when developing section 3.3, top-k selection without dispatch, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 3.3 - Top-k selection without dispatch
...
Develop only Section 3.3 of *MoE Models from Scratch*: **Top-k selection without dispatch**.

- Chapter: 3. Experts, routers, and the first MoE layer
- Target section file: `latex_book_skeleton/chapters/ch03/sections/03-top-k-selection-without-dispatch.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch03/fig-top-k-selection-without-dispatch.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch03/tab-top-k-selection-without-dispatch.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch03/eq-top-k-selection-without-dispatch.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch03/lst-top-k-selection-without-dispatch.tex`
- Required diagnostic: Verify that every token has exactly K selected expert IDs.
...
```text
Develop Section 3.3, "Top-k selection without dispatch", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```
...
# Detailed section development brief: Chapter 3.3 - Top-k selection without dispatch
...
| Chapter | 3. Experts, routers, and the first MoE layer |
| Section | 3.3. Top-k selection without dispatch |
| Section label slug | `ch03-top-k-selection-without-dispatch` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch03/sections/03-top-k-selection-without-dispatch.tex` |
| Planned section include | `\input{chapters/ch03/sections/03-top-k-selection-without-dispatch}` |
| Previous handoff | Chapter 3.2 - Router scores and tensor shapes |
| Next handoff | Chapter 3.4 - Naive token-by-token dispatch |
...
**Mini-example to anchor the section:** Compute top-2 expert IDs and scores for a four-token, four-expert score table.
...
**This section's bridge sentence:** Selection tells us where tokens should go; dispatch actually sends them there.
...
1. **Opening paragraph.** State the concrete problem: Separate the selection problem from expert execution so readers can inspect router behavior first. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Compute top-2 expert IDs and scores for a four-token, four-expert score table.
3. **Visual explanation.** Insert `\input{figures/ch03/fig-top-k-selection-without-dispatch}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch03/eq-top-k-selection-without-dispatch}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch03/lst-top-k-selection-without-dispatch}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch03/tab-top-k-selection-without-dispatch}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch03/fig-top-k-selection-without-dispatch.tex` |
| Include command | `\input{figures/ch03/fig-top-k-selection-without-dispatch}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch03-top-k-selection-without-dispatch` |
| Caption intent | Score matrix with the top-k entries highlighted in each row. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Score matrix with the top-k entries highlighted in each row.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch03/tab-top-k-selection-without-dispatch.tex` |
| Include command | `\input{tables/ch03/tab-top-k-selection-without-dispatch}` |
| Label | `tab:ch03-top-k-selection-without-dispatch` |
| Caption intent | Manual top-k results for the toy score table. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Top-k selection without dispatch"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch03/eq-top-k-selection-without-dispatch.tex` |
| Include command | `\input{equations/ch03/eq-top-k-selection-without-dispatch}` |
| Label | `eq:ch03-top-k-selection-without-dispatch` |
| Equation intent | S_t = topk(score_t, K), one selected set per token. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch03/lst-top-k-selection-without-dispatch.tex` |
| Include command | `\input{listings/ch03/lst-top-k-selection-without-dispatch}` |
| Label | `lst:ch03-top-k-selection-without-dispatch` |
| Listing intent | torch.topk example that returns topk_score and topk_idx. |

**Code objects to develop or reference:**
- `topk_router helper`
- `selected gate normalization test`
...
2. Add only the minimal new object or function needed for **Top-k selection without dispatch**.
3. Write a CPU smoke test using the mini-example: Compute top-2 expert IDs and scores for a four-token, four-expert score table.
...
**Expected code deliverable:** torch.topk example that returns topk_score and topk_idx.
