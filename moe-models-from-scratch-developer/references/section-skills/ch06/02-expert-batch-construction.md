---
name: moe-ch06-s02-expert-batch-construction
description: use when developing section 6.2, expert batch construction, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 6.2 - Expert batch construction
...
Develop only Section 6.2 of *MoE Models from Scratch*: **Expert batch construction**.

- Chapter: 6. Vectorized dispatch, capacity, and routing efficiency
- Target section file: `latex_book_skeleton/chapters/ch06/sections/02-expert-batch-construction.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch06/fig-expert-batch-construction.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch06/tab-expert-batch-construction.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch06/eq-expert-batch-construction.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch06/lst-expert-batch-construction.tex`
- Required diagnostic: Ensure each accepted route has exactly one expert slot.
...
```text
Develop Section 6.2, "Expert batch construction", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 6.2 - Expert batch construction

Develop only Chapter 6.2, **Expert batch construction**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch06/02-expert-batch-construction.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch06/sections/02-expert-batch-construction.tex`
...
# Detailed section development brief: Chapter 6.2 - Expert batch construction
...
| Chapter | 6. Vectorized dispatch, capacity, and routing efficiency |
| Section | 6.2. Expert batch construction |
| Section label slug | `ch06-expert-batch-construction` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch06/sections/02-expert-batch-construction.tex` |
| Planned section include | `\input{chapters/ch06/sections/02-expert-batch-construction}` |
| Previous handoff | Chapter 6.1 - Flattened tokens as the dispatch unit |
| Next handoff | Chapter 6.3 - Capacity factors and overflow policy |
...
**Mini-example to anchor the section:** Create an expert batch tensor with shape (E, capacity, D).
...
**This section's bridge sentence:** Fixed-size expert batches require a capacity rule.
...
1. **Opening paragraph.** State the concrete problem: Build dense per-expert mini-batches from sparse token assignments. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Create an expert batch tensor with shape (E, capacity, D).
3. **Visual explanation.** Insert `\input{figures/ch06/fig-expert-batch-construction}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch06/eq-expert-batch-construction}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch06/lst-expert-batch-construction}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch06/tab-expert-batch-construction}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch06/fig-expert-batch-construction.tex` |
| Include command | `\input{figures/ch06/fig-expert-batch-construction}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch06-expert-batch-construction` |
| Caption intent | Sparse token assignments packed into a fixed-size expert batch grid. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Sparse token assignments packed into a fixed-size expert batch grid.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch06/tab-expert-batch-construction.tex` |
| Include command | `\input{tables/ch06/tab-expert-batch-construction}` |
| Label | `tab:ch06-expert-batch-construction` |
| Caption intent | expert_batch, combine_weights, token_indices, and slot_indices shapes. |
| Required columns | Tensor name | Shape | Created by | Used by | Failure check |

**Planned table rows:**
- row 1: define the primary object for "Expert batch construction"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch06/eq-expert-batch-construction.tex` |
| Include command | `\input{equations/ch06/eq-expert-batch-construction}` |
| Label | `eq:ch06-expert-batch-construction` |
| Equation intent | expert_batch[i, slot, :] = x_flat[token_idx]. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch06/lst-expert-batch-construction.tex` |
| Include command | `\input{listings/ch06/lst-expert-batch-construction}` |
| Label | `lst:ch06-expert-batch-construction` |
| Listing intent | pack_tokens_by_expert function for top-k assignments. |

**Code objects to develop or reference:**
- `build_expert_batches`
- `dispatch record arrays`
...
2. Add only the minimal new object or function needed for **Expert batch construction**.
3. Write a CPU smoke test using the mini-example: Create an expert batch tensor with shape (E, capacity, D).
...
**Expected code deliverable:** pack_tokens_by_expert function for top-k assignments.
