---
name: moe-ch06-s03-capacity-factors-and-overflow-policy
description: use when developing section 6.3, capacity factors and overflow policy, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 6.3 - Capacity factors and overflow policy
...
Develop only Section 6.3 of *MoE Models from Scratch*: **Capacity factors and overflow policy**.

- Chapter: 6. Vectorized dispatch, capacity, and routing efficiency
- Target section file: `latex_book_skeleton/chapters/ch06/sections/03-capacity-factors-and-overflow-policy.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch06/fig-capacity-factors-and-overflow-policy.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch06/tab-capacity-factors-and-overflow-policy.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch06/eq-capacity-factors-and-overflow-policy.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch06/lst-capacity-factors-and-overflow-policy.tex`
- Required diagnostic: Log overflow route count and overflow fraction.
...
```text
Develop Section 6.3, "Capacity factors and overflow policy", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 6.3 - Capacity factors and overflow policy

Develop only Chapter 6.3, **Capacity factors and overflow policy**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch06/03-capacity-factors-and-overflow-policy.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch06/sections/03-capacity-factors-and-overflow-policy.tex`
...
# Detailed section development brief: Chapter 6.3 - Capacity factors and overflow policy
...
| Chapter | 6. Vectorized dispatch, capacity, and routing efficiency |
| Section | 6.3. Capacity factors and overflow policy |
| Section label slug | `ch06-capacity-factors-and-overflow-policy` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch06/sections/03-capacity-factors-and-overflow-policy.tex` |
| Planned section include | `\input{chapters/ch06/sections/03-capacity-factors-and-overflow-policy}` |
| Previous handoff | Chapter 6.2 - Expert batch construction |
| Next handoff | Chapter 6.4 - Unpacking and weighted combine |
...
**Mini-example to anchor the section:** Use N=12, E=4, K=2 and capacity factors 1.0 and 1.25.
...
**This section's bridge sentence:** After expert batches are processed, outputs must be unpacked and combined.
...
1. **Opening paragraph.** State the concrete problem: Explain how capacity controls memory and what the implementation does with overflow routes. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Use N=12, E=4, K=2 and capacity factors 1.0 and 1.25.
3. **Visual explanation.** Insert `\input{figures/ch06/fig-capacity-factors-and-overflow-policy}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch06/eq-capacity-factors-and-overflow-policy}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch06/lst-capacity-factors-and-overflow-policy}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch06/tab-capacity-factors-and-overflow-policy}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch06/fig-capacity-factors-and-overflow-policy.tex` |
| Include command | `\input{figures/ch06/fig-capacity-factors-and-overflow-policy}` |
| Type | TikZ schematic wrapper |
| Label | `fig:ch06-capacity-factors-and-overflow-policy` |
| Caption intent | Capacity slots per expert with accepted, padded, and overflow routes. |
| Wrapper contract | The figure file must contain a `tikzpicture` environment, caption, and label. |

**Figure content plan:** Capacity slots per expert with accepted, padded, and overflow routes.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch06/tab-capacity-factors-and-overflow-policy.tex` |
| Include command | `\input{tables/ch06/tab-capacity-factors-and-overflow-policy}` |
| Label | `tab:ch06-capacity-factors-and-overflow-policy` |
| Caption intent | Capacity factor scenarios and their accepted route counts. |
| Required columns | Capacity term | Symbol/code name | Meaning | Edge case | Diagnostic |

**Planned table rows:**
- row 1: define the primary object for "Capacity factors and overflow policy"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch06/eq-capacity-factors-and-overflow-policy.tex` |
| Include command | `\input{equations/ch06/eq-capacity-factors-and-overflow-policy}` |
| Label | `eq:ch06-capacity-factors-and-overflow-policy` |
| Equation intent | capacity = ceil(capacity_factor * N*K / E). |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch06/lst-capacity-factors-and-overflow-policy.tex` |
| Include command | `\input{listings/ch06/lst-capacity-factors-and-overflow-policy}` |
| Label | `lst:ch06-capacity-factors-and-overflow-policy` |
| Listing intent | compute_expert_capacity and overflow mask code. |

**Code objects to develop or reference:**
- `capacity_factor_to_slots`
- `overflow masks`
...
2. Add only the minimal new object or function needed for **Capacity factors and overflow policy**.
3. Write a CPU smoke test using the mini-example: Use N=12, E=4, K=2 and capacity factors 1.0 and 1.25.
...
**Expected code deliverable:** compute_expert_capacity and overflow mask code.
...
- State explicitly whether overflow tokens use a residual passthrough, zero contribution, or are excluded from expert output.
