---
name: moe-ch09-s04-dynamic-bias-update-from-expert-load
description: use when developing section 9.4, dynamic bias update from expert load, for the latex-first book moe models from scratch. triggers include drafting this section, replacing its artifact wrappers, updating its local code, running its diagnostic, or proofreading its handoff.
---

# Section developer: Chapter 9.4 - Dynamic bias update from expert load
...
Develop only Section 9.4 of *MoE Models from Scratch*: **Dynamic bias update from expert load**.

- Chapter: 9. Auxiliary-loss-free load balancing
- Target section file: `latex_book_skeleton/chapters/ch09/sections/04-dynamic-bias-update-from-expert-load.tex`
- Planned figure wrapper: `latex_book_skeleton/figures/ch09/fig-dynamic-bias-update-from-expert-load.tex`
- Planned table wrapper: `latex_book_skeleton/tables/ch09/tab-dynamic-bias-update-from-expert-load.tex`
- Planned equation wrapper: `latex_book_skeleton/equations/ch09/eq-dynamic-bias-update-from-expert-load.tex`
- Planned listing wrapper: `latex_book_skeleton/listings/ch09/lst-dynamic-bias-update-from-expert-load.tex`
- Required diagnostic: Track bias values over training beside expert loads.
...
```text
Develop Section 9.4, "Dynamic bias update from expert load", for MoE Models from Scratch. Use this skill markdown as the complete handoff. Produce LaTeX-first section prose, update only the artifact wrappers listed here, preserve every artifact through \input, and include a runnable or near-runnable diagnostic tied to the planned code path.
```

## Source session starter

# Session starter: Chapter 9.4 - Dynamic bias update from expert load

Develop only Chapter 9.4, **Dynamic bias update from expert load**, for *MoE Models from Scratch*.

Use the detailed brief at:

`section_development_briefs/ch09/04-dynamic-bias-update-from-expert-load.md`

Must produce LaTeX section prose at:

`latex_book_skeleton/chapters/ch09/sections/04-dynamic-bias-update-from-expert-load.tex`
...
# Detailed section development brief: Chapter 9.4 - Dynamic bias update from expert load
...
| Chapter | 9. Auxiliary-loss-free load balancing |
| Section | 9.4. Dynamic bias update from expert load |
| Section label slug | `ch09-dynamic-bias-update-from-expert-load` |
| Section `.tex` target | `latex_book_skeleton/chapters/ch09/sections/04-dynamic-bias-update-from-expert-load.tex` |
| Planned section include | `\input{chapters/ch09/sections/04-dynamic-bias-update-from-expert-load}` |
| Previous handoff | Chapter 9.3 - Biased selection and unbiased combine |
| Next handoff | Chapter 9.5 - Comparing balancing methods |
...
**Mini-example to anchor the section:** Compare observed loads [8,2,1,1] to target load 3 and update each bias.
...
**This section's bridge sentence:** Now we can compare auxiliary balancing and bias balancing using the same model and metrics.
...
1. **Opening paragraph.** State the concrete problem: Add a simple no-gradient update that nudges underused experts up and overused experts down. Do not begin with a survey paragraph.
2. **Motivating example.** Use the mini-example exactly or as a close variant: Compare observed loads [8,2,1,1] to target load 3 and update each bias.
3. **Visual explanation.** Insert `\input{figures/ch09/fig-dynamic-bias-update-from-expert-load}` immediately after the setup.
...
5. **Mechanics and equation.** Insert `\input{equations/ch09/eq-dynamic-bias-update-from-expert-load}` and explain why it implements the local objective.
6. **Implementation.** Insert `\input{listings/ch09/lst-dynamic-bias-update-from-expert-load}` and annotate only the lines where a reader could make a shape, routing, or gradient mistake.
7. **Diagnostic.** Insert `\input{tables/ch09/tab-dynamic-bias-update-from-expert-load}` near the diagnostic discussion if the table is a contract/check table; otherwise place it before code as a notation table.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/figures/ch09/fig-dynamic-bias-update-from-expert-load.tex` |
| Include command | `\input{figures/ch09/fig-dynamic-bias-update-from-expert-load}` |
| Type | Generated PDF/plot wrapper |
| Label | `fig:ch09-dynamic-bias-update-from-expert-load` |
| Caption intent | Feedback loop from observed expert histogram to router bias update. |
...
**Figure content plan:** Feedback loop from observed expert histogram to router bias update.
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/tables/ch09/tab-dynamic-bias-update-from-expert-load.tex` |
| Include command | `\input{tables/ch09/tab-dynamic-bias-update-from-expert-load}` |
| Label | `tab:ch09-dynamic-bias-update-from-expert-load` |
| Caption intent | Bias update hyperparameters: gamma, target load, update interval, clamp range. |
| Required columns | Concept | Local definition | Shape/code object | Why it matters | Check |

**Planned table rows:**
- row 1: define the primary object for "Dynamic bias update from expert load"
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/equations/ch09/eq-dynamic-bias-update-from-expert-load.tex` |
| Include command | `\input{equations/ch09/eq-dynamic-bias-update-from-expert-load}` |
| Label | `eq:ch09-dynamic-bias-update-from-expert-load` |
| Equation intent | b_i <- b_i + gamma if load_i < target, otherwise b_i <- b_i - gamma. |
...
| Field | Requirement |
|---|---|
| Path | `latex_book_skeleton/listings/ch09/lst-dynamic-bias-update-from-expert-load.tex` |
| Include command | `\input{listings/ch09/lst-dynamic-bias-update-from-expert-load}` |
| Label | `lst:ch09-dynamic-bias-update-from-expert-load` |
| Listing intent | update_router_bias(load, target, gamma) implemented under torch.no_grad(). |
...
2. Add only the minimal new object or function needed for **Dynamic bias update from expert load**.
3. Write a CPU smoke test using the mini-example: Compare observed loads [8,2,1,1] to target load 3 and update each bias.
...
**Expected code deliverable:** update_router_bias(load, target, gamma) implemented under torch.no_grad().
